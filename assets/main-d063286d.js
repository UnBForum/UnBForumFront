import{s as p,t as i,a as Le,j as e,I as Te,u as H,r as a,F as Ie,S as ie,P as Ue,b as pe,B as j,c as K,n as he,q as Pe,d as me,e as Z,A as ge,V as qe,H as xe,T as fe,f as Me,C as re,g as O,h as Fe,i as Be,k as ae,D as ne,l as Ne,m as Ve,o as Re,p as Ze,v as Xe,w as He,x as Oe,y as le,z as Je,M as G,E as Ge,G as Ce,O as ke,J as we,K as ce,L as Ke,N as Ye,Q as We,R as De,U as $e,W as Se,X as _e,Y as et,Z as tt,_ as ot,$ as q,a0 as st,a1 as it,a2 as nt,a3 as rt,a4 as at,a5 as lt}from"./vendor-6c544215.js";(function(){const o=document.createElement("link").relList;if(o&&o.supports&&o.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))r(n);new MutationObserver(n=>{for(const c of n)if(c.type==="childList")for(const d of c.addedNodes)d.tagName==="LINK"&&d.rel==="modulepreload"&&r(d)}).observe(document,{childList:!0,subtree:!0});function s(n){const c={};return n.integrity&&(c.integrity=n.integrity),n.referrerPolicy&&(c.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?c.credentials="include":n.crossOrigin==="anonymous"?c.credentials="omit":c.credentials="same-origin",c}function r(n){if(n.ep)return;n.ep=!0;const c=s(n);fetch(n.href,c)}})();window.global||(window.global=window);const ct=p.main`
  width: 70%;
  display: flex;
  align-items: flex-start;
  justify-content: space-between;
  gap: 1rem;
  margin-top: 2rem;

  @media (max-width: 768px) {
    flex-direction: column;
    align-items: center;
    justify-content: center;
    width: 100%;
    margin-top: 1rem;
  }
`,dt=p.section`
  width: 60%;
  display: flex;
  flex-direction: column;
  justify-content: flex-start;
  gap: 1rem;
  margin-bottom: 1rem;

  & > div[role='button'] {
    width: 100%;
    font-size: 1rem;
    font-weight: bold;
    color: ${i.colors.text[50]};
    transition: all 0.2s;

    &:hover {
      box-shadow: 0 0 10px ${i.colors.success[600]};
    }

    & > svg {
      width: 0.6rem;
      height: 0.6rem;
    }
  }

  @media (max-width: 768px) {
    width: 90%;
    gap: 0.7rem;

    & > div[role='button'] {
      font-size: 0.9rem;
    }
  }
`,ut=p.main`
  width: 100%;
  height: 60vh;
  display: flex;
  flex-direction: column;
  /* gap: 1rem; */
  /* background-color: #f3f3f3; */
  overflow-y: auto;

  @media (max-width: 768px) {
    height: auto;
    /* overflow-y: hidden; */
  }
`,pt=p.section`
  width: 40%;
  display: flex;
  flex-direction: column;
  align-items: flex-start;
  justify-content: flex-start;
  gap: 1rem;

  & > div[role='button'] {
    width: 100%;
    font-size: 1rem;
    font-weight: bold;
    color: ${i.colors.text[50]};
    transition: all 0.2s;

    &:hover {
      box-shadow: 0 0 10px ${i.colors.success[600]};
    }

    & > svg {
      width: 0.6rem;
      height: 0.6rem;
    }
  }

  @media (max-width: 768px) {
    width: 90%;
    margin-bottom: 1rem;

    & > div[role='button'] {
      font-size: 0.9rem;
    }
  }
`,ht=p.aside`
  width: 100%;
  height: 75vh;
  display: flex;
  flex-direction: column;
  align-items: center;
  gap: 1.5rem;

  background-color: ${i.colors.info[50]};
  box-shadow: 0 0 10px ${i.colors.primary[800]};
  border-radius: 5px;
  padding: 0.7rem 0;

  overflow-y: auto;

  #favorite-title-container {
    width: 100%;
    display: flex;
    flex-direction: row;
    align-items: center;
    justify-content: center;
    gap: 0.7rem;
  }

  #title-favorites {
    font-size: 1.5rem;
    font-weight: 600;
    color: ${i.colors.rose[600]};
  }

  @media (max-width: 768px) {
    height: auto;
  }
`,mt=p.div`
  width: 100%;
  background-color: ${i.colors.info[700]};
  display: flex;
  align-items: center;
  flex-direction: column;
  padding: 0.7rem 0.7rem;
  border-radius: 5px;
  gap: 1rem;

  color: white;

  @media (max-width: 768px) {
    flex-direction: column;
    gap: 0.5rem;
  }
`,gt=p.div`
  width: 100%;
  display: flex;
  flex-direction: row;
  align-items: center;
  justify-content: space-between;
  gap: 1rem;

  & > div {
    width: 95%;
  }

  @media (max-width: 768px) {
    gap: 0.5rem;
  }
`,xt=p.div`
  width: 100%;
  display: flex;
  flex-direction: row;
  align-items: center;
  justify-content: space-between;
  gap: 1rem;

  & > div {
    width: 95%;
  }

  @media (max-width: 768px) {
    /* width: 100%; */
    gap: 0.5rem;
  }
`,ft=p.div`
  width: 100%;
  display: flex;
  flex-direction: row;
  align-items: center;
  justify-content: space-between;
  gap: 1rem;

  & > div {
    width: 95%;
  }

  @media (max-width: 768px) {
    /* width: 100%; */
    gap: 0.5rem;
  }
`,be=[{label:"Mais recentes",value:"-created_at",color:"#000"},{label:"Mais populares",value:"-rating",color:"#000"},{label:"Mais comentários",value:"-comments_count",color:"#000"}],N=Le.create({baseURL:"https://unbforum-backend-4b05406a8bbf.herokuapp.com/"}),de=async()=>await N.get("categories/"),Ct=async t=>await N.post("categories/",t),wt=async t=>await N.delete(`categories/${t}/`),bt=async t=>await N.patch(`categories/${t.id}/`,{...t}),jt=p.div`
  width: 100%;
  display: flex;
  flex-direction: column;
  /* gap: 0.3rem; */

  #error-feedback {
    width: 100%;
    display: flex;
    flex-direction: row;
    justify-content: flex-start;
    align-items: center;
    gap: 1rem;

    /* font-size: ; */

    & > svg {
      width: 0.7rem;
    }
  }
`,yt=p.label`
  display: flex;
  flex-direction: row;
  gap: 0.3rem;

  font-size: ${({fontSize:t})=>t};
  line-height: 1.5rem;
  color: ${i.colors.text[500]};

  #required {
    color: ${i.colors.danger[600]};
  }

  @media (max-width: 768px) {
    font-size: ${({fontSize:t})=>Number(t.split("rem")[0])-.2+"rem"};
  }
`;function V({label:t="",inputType:o="text",placeholder:s,accessibilityLabel:r,size:n="md",fontSize:c="0.9rem",name:d,isRequired:u=!0,isEditing:l=!0,value:h=void 0,backgroundColor:m=null,height:C=null,onChange:g,...x}){return e.jsxs(jt,{children:[t&&e.jsxs(yt,{fontSize:c,children:[t,u&&e.jsx("p",{id:"required",children:"*"})]}),e.jsx(Te,{height:C||null,type:o,variant:l?"outline":"filled",size:n,backgroundColor:m||null,isRequired:u,value:h,placeholder:s,editable:l,accessibilityLabel:r,onChangeText:w=>g(d,w),...x})]})}function vt({onChangeCategoriesFilter:t,onChangeOrderBy:o,onChangeSearchInput:s,handleSearch:r}){const n=H("(max-width: 768px)"),[c,d]=a.useState([]),u={control:l=>({...l,backgroundColor:i.colors.white}),option:(l,{data:h,isMulti:m,isDisabled:C,isFocused:g,isSelected:x})=>{const w=K(h.color);return{...l,backgroundColor:C?void 0:x?m?h.color:w.alpha(.2).css():g?w.alpha(.1).css():void 0,color:C?"#ccc":x&&m?K.contrast(w,"white")>2?"white":"black":h.color,cursor:C?"not-allowed":"default",":active":{...l[":active"],backgroundColor:C?void 0:x?h.color:w.alpha(.3).css()}}},multiValue:(l,{data:h})=>{const m=K(h.color);return{...l,backgroundColor:m.alpha(.1).css()}},multiValueLabel:(l,{data:h})=>({...l,color:h.color}),multiValueRemove:(l,{data:h})=>({...l,color:h.color,":hover":{backgroundColor:h.color,color:"white"}}),input:l=>({...l,"input:focus":{boxShadow:"none"}})};return a.useEffect(()=>{de().then(l=>{d(l.data)})},[]),e.jsxs(mt,{children:[e.jsxs(gt,{children:[e.jsx(Ie,{size:20,color:i.colors.tertiary[300]}),e.jsx(ie,{isMulti:!0,closeMenuOnSelect:!1,options:c.map(l=>({value:l.id.toString(),label:l.name,color:l.color})),styles:u,onChange:t,placeholder:"Filtrar por categorias"})]}),e.jsxs(ft,{children:[e.jsx(Ue,{size:22,color:i.colors.tertiary[300]}),e.jsx(ie,{isMulti:!1,defaultValue:be[0],options:be,placeholder:"Ordenar Tópicos",styles:u,onChange:o})]}),e.jsxs(xt,{children:[e.jsx(pe,{color:i.colors.tertiary[300],size:"22"}),e.jsx(V,{name:"topic-search",height:"2.4rem",size:n?"md":"lg",backgroundColor:i.colors.white,accessibilityLabel:"Buscar tópicos",placeholder:"Buscar tópicos...",inputType:"text",onChange:s,InputRightElement:e.jsx(j,{rounded:"none",w:"1/12",h:"full",onPress:r,children:e.jsx(pe,{color:i.colors.white,size:"22"})})})]})]})}const Nt=(t,o)=>t?"0.9rem":o?"1rem":"0.9rem",kt=(t,o)=>t?"0.7rem":o?"0.8rem":"0.7rem",Dt=p.div`
  width: 100%;
  display: flex;
  /* padding: 0.5rem; */
  gap: 0.5rem;
  margin-top: 1rem;
`,St=p.div`
  width: ${({isComment:t})=>t?"15%":"10%"};
  display: flex;
  flex-direction: column;
  align-items: center;
  justify-content: ${({isComment:t})=>"flex-start"};
  gap: ${({isInsideTopic:t,isComment:o})=>t&&!o?"1rem":"0.4rem"};

  & > p {
    font-weight: ${({isComment:t})=>t?"700":"900"};
    font-size: ${({isComment:t})=>t?"1.2rem":"1.4rem"};
    color: ${i.colors.primary[900]};
  }

  #markButton {
    transition: 0.5s all ease;
  }

  #markButton:hover > svg {
    filter: drop-shadow(1px 1px 2px rgba(0, 0, 0, 0.5));
  }

  #markButton:focus {
    box-shadow: none;
  }

  #reactionButton {
    & > svg {
      width: ${({isComment:t})=>t?"1.2rem":"1.5rem"};
      height: ${({isComment:t})=>t?"1.2rem":"1.5rem"};
    }
  }

  @media (max-width: 768px) {
    width: ${({isInsideTopic:t})=>t?"10%":"15%"};
    gap: 0.2rem;

    & > p {
      font-weight: 800;
      font-size: 1.1rem;
    }

    #reactionButton {
      & > svg {
        width: 1rem;
        height: 1rem;
      }
    }
  }
`,Qt=p.div`
  width: 100%;
  display: flex;
`,zt=p.div`
  width: ${({isComment:t})=>t?"85%":"100%"};
  display: flex;
  flex-direction: column;
  gap: ${({isComment:t})=>t?"0.4rem":"0.6rem"};

  & > p {
    width: ${({isInsideTopic:t})=>t?"90%":"100%"};
    text-align: left;
    font-weight: ${({isComment:t,isInsideTopic:o})=>"400"};
    font-size: ${({isComment:t,isInsideTopic:o})=>Nt(t,o)};
    color: ${i.colors.black};
    line-height: ${({isComment:t})=>t?"1.7":"2"};
  }

  & > span {
    font-size: 0.8rem;
    font-weight: ${({isComment:t,isInsideTopic:o})=>"400"};
    color: ${i.colors.black};
    line-height: 1;
  }

  /* &:focus {
    box-shadow: none;
  } */

  #post-title {
    font-size: 2.5rem;
    font-weight: 600;
  }

  @media (max-width: 768px) {
    width: 100%;
    gap: 0.4rem;

    & > p {
      width: 100%;
      font-size: ${({isComment:t,isInsideTopic:o})=>kt(t,o)};
      line-height: 1.7;
    }

    & > span {
      font-size: 0.65rem;
    }

    #post-title {
      font-size: 1.2rem;
      font-weight: 600;
    }
  }
`,At=p.div`
  width: 100%;
  display: flex;
  align-items: ${({isInsideTopic:t})=>"center"};
  flex-direction: row;
  margin-bottom: 1rem;

  & > p {
    font-weight: ${({isInsideTopic:t})=>t?500:600};
    font-size: 1rem;
    text-align: left;
    color: ${i.colors.black};
  }

  @media (max-width: 768px) {
    & > p {
      font-size: 0.8rem;
    }
  }
`,Et=p.div`
  width: 100%;
  display: flex;
  flex-wrap: wrap;
  flex-direction: row;
  gap: 0.7rem;

  #post-badge-text {
    color: ${i.colors.text[50]};
    font-size: 1rem;
    line-height: 1.5;
    font-weight: 600;
  }

  @media (max-width: 768px) {
    gap: 0.4rem;

    #post-badge-text {
      font-size: 0.7rem;
      line-height: 1.3;
    }
  }
`,Lt=p(j)`
  width: ${({isInsideTopic:t})=>t?"50%":"90%"};
  /* display: flex;
  flex-direction: row;
  align-items: center;
  justify-content: flex-start; */
  gap: 0.5rem;
  transition: all 0.5s ease;

  svg {
    color: ${i.colors.primary[600]};
  }

  #comment-button {
    font-size: 1.4rem;
    font-weight: 600;
    color: ${i.colors.primary[600]};
  }

  ${({isInsideTopic:t})=>t?he`
          cursor: initial;
        `:he`
          &:hover {
            /* filter: brightness(50%); */

            & > p {
              text-shadow: 1px 1px 2px rgba(0, 0, 0, 0.5);
              color: ${i.colors.primary[400]};
            }

            & > svg {
              color: ${i.colors.primary[400]};
              filter: drop-shadow(1px 1px 2px rgba(0, 0, 0, 0.5));
            }
          }
        `}

  &:focus {
    box-shadow: none;
  }

  @media (max-width: 768px) {
    gap: 0.3rem;

    & > p {
      font-size: 1rem;
    }
  }
`;p.div`
  width: 40%;
  display: flex;
  flex-direction: row;
  align-items: center;
  justify-content: space-between;
  gap: 1rem;

  border-radius: 5px;
  background-color: ${i.colors.tertiary[100]};

  & > p {
    font-size: 1.5rem;
    font-weight: 800;
    color: ${i.colors.primary[800]};
    text-shadow: 0 0 2px rgba(255, 255, 255);
  }

  & > div[role='button']:focus {
    box-shadow: none;
  }

  & > div[role='button']:hover {
    /* box-shadow: none; */
  }

  @media (max-width: 768px) {
    width: 100%;
  }
`;const Tt=async t=>await N.get(`topics/?${Pe.stringify(t,{arrayFormat:"comma"})}`),It=async t=>await N.get(`topics/${t}`),Ut=async t=>await N.post("topics",t),Pt=async t=>await N.post(`topics/${t}/upvote`),qt=async t=>await N.post(`topics/${t}/downvote`),Mt=async t=>await N.post(`topics/${t}/save`),Ft=async t=>await N.post(`topics/${t}/unsave`),Bt=async()=>await N.get("users/me/saved_topics/"),oe="moderator",se="administrator",Qe=a.createContext(null),Vt=({children:t})=>{const[o,s]=a.useState(""),[r,n]=a.useState([]),[c,d]=a.useState(""),[u,l]=a.useState("");a.useEffect(()=>{const g=localStorage.getItem("@UnBForum:token");if(console.log("Checando token...",g),g){const x=me(g.split(" ")[1]);N.defaults.headers.common.Authorization=`${g}`,s(g.split(" ")[1]),n(x.scopes),d(x.sub),l(x.name)}},[]);const h=a.useCallback(g=>{localStorage.setItem("@UnBForum:token",`Bearer ${g}`);const x=me(g);N.defaults.headers.common.Authorization=`Bearer ${g}`,s(g),n(x.scopes),d(x.sub),l(x.name)},[]),m=a.useCallback(g=>r.some(x=>x===g),[r]),C=a.useCallback(()=>{s(""),n([]),d(""),l(""),N.defaults.headers.common.Authorization=null,localStorage.removeItem("@UnBForum:token")},[]);return e.jsx(Qe.Provider,{value:{token:o,name:u,email:c,scopes:r,changeToken:h,checkScopePermissions:m,logoutUser:C,setName:l},children:t})};function J(){const t=a.useContext(Qe);if(!t)throw new Error("useUser must be used within an UserThemeProvider");return t}const v=({id:t,status:o,variant:s="left-accent",title:r,description:n,isClosable:c=!1,...d})=>{const u=Z();return e.jsx(ge,{maxWidth:"100%",alignSelf:"center",flexDirection:"row",status:o||"error",variant:s,...d,children:e.jsxs(qe,{space:1,flexShrink:1,w:"100%",children:[e.jsxs(xe,{flexShrink:1,alignItems:"center",justifyContent:"space-between",children:[e.jsxs(xe,{space:2,flexShrink:1,alignItems:"center",children:[e.jsx(ge.Icon,{}),e.jsx(fe,{fontSize:"md",fontWeight:"medium",flexShrink:1,color:s==="solid"?"lightText":s!=="outline"?"darkText":null,children:r})]}),c?e.jsx(Me,{variant:"unstyled",icon:e.jsx(re,{size:"3"}),_icon:{color:s==="solid"?"lightText":"darkText"},onPress:()=>u.close(t)}):null]}),e.jsx(fe,{px:"6",color:s==="solid"?"lightText":s!=="outline"?"darkText":null,children:n})]})})},Rt=async(t,o)=>await N.post(`topics/${o}/comments/`,{content:t}),Zt=async t=>await N.get(`topics/${t}/comments/`),Xt=async(t,o)=>await N.post(`topics/${o}/comments/${t}/upvote`),Ht=async(t,o)=>await N.post(`topics/${o}/comments/${t}/downvote`),Ot=t=>`https://unbforum-backend-4b05406a8bbf.herokuapp.com/files/download/${t.upload_path}`;function ze(t){const o=t.split(" ");if(o.length>=2){const s=o[0],r=o[o.length-1],n=s[0],c=r[0];return n.toUpperCase()+c.toUpperCase()}else return o[0][0].toUpperCase()}function ee({id:t,isInsideTopic:o=!1,isComment:s=!1,title:r="",author:n,content:c,topicId:d=null,currentRating:u=0,rating:l=0,files:h=[],categories:m=[],commentsCount:C=0,isSave:g=!1}){const{token:x}=J(),w=Z(),Q=O(),S=H("(max-width: 768px)"),[k,A]=a.useState(l),[U,z]=a.useState(g),[E,b]=a.useState(u);a.useEffect(()=>{b(u),A(l),z(g)},[u,l,g]);function f(){Q(`/topic/${t}`)}const y=()=>{w.show({placement:"top-right",render:()=>e.jsx(v,{id:"user-unlogged-error",title:"Opa!",description:"Para interagir com um tópico ou comentário, você precisa fazer login!",status:""})})};async function T(D,M){if(D.preventDefault(),!x){y();return}if(M>0){const W=await(s?Xt(t,d):Pt(t));A(W.data.rating),b(E>0?0:1)}else{const W=await(s?Ht(t,d):qt(t));A(W.data.rating),b(E<0?0:-1)}}function L(D){if(D.preventDefault(),!x){y();return}U?Ft(t).then(M=>{z(!1)}).catch(M=>{}):Mt(t).then(M=>{z(!0)}).catch(M=>{})}function F(){switch(U){case!0:return e.jsx(Ze,{size:"30",color:i.colors.darkBlue[400]});case!1:return e.jsx(Re,{size:"30",color:i.colors.darkBlue[400]})}}return e.jsxs(e.Fragment,{children:[e.jsxs(Dt,{children:[e.jsxs(St,{isInsideTopic:o,isComment:s,children:[e.jsx("button",{id:"reactionButton",onClick:D=>T(D,1),children:e.jsx(Fe,{color:E>0?i.colors.tertiary[500]:i.colors.muted[500],size:"18"})}),e.jsx("p",{children:k}),e.jsx("button",{id:"reactionButton",onClick:D=>T(D,-1),children:e.jsx(Be,{color:E<0?i.colors.tertiary[500]:i.colors.muted[500],size:"18"})}),!s&&e.jsx("button",{id:"markButton",onClick:D=>L(D),children:F()})]}),e.jsx(Qt,{children:e.jsxs(zt,{isInsideTopic:o,isComment:s,children:[o&&e.jsx("h1",{id:"post-title",children:r}),e.jsxs(At,{isInsideTopic:o,children:[e.jsx(ae,{bg:s?"lime.600":"emerald.600",marginRight:"0.8rem",size:s?"xs":"sm",children:ze(n)}),e.jsxs("p",{children:[n,!o&&!s&&` - ${r}`]})]}),e.jsx("p",{dangerouslySetInnerHTML:{__html:c.replace(/\n/g,"<br>")}}),h.length>0&&e.jsx(ne,{width:"80%"}),h.map((D,M)=>e.jsxs("span",{children:["Para acessar o arquivo ",`${M+1}: `,e.jsx("a",{href:Ot(D),target:"_blank",rel:"noreferrer",children:"clique aqui"})]},D.id)),!s&&e.jsx(Et,{children:m.map(D=>e.jsx(Ne,{variant:"solid",bg:D.color,alignSelf:"center",size:"md",textDecoration:"solid .8rem bold",children:e.jsx("p",{id:"post-badge-text",children:D.name})},D.id))}),!o&&!s&&e.jsx(Lt,{variant:"outline",width:"95%",size:"xs",_text:{fontSize:S?"0.8rem":"1rem"},isInsideTopic:o,onPress:o?()=>{}:()=>f(),rightIcon:e.jsx(Ve,{size:S?"18":"20",color:i.colors.primary[700]}),_icon:{marginLeft:"1rem"},children:`Ver comentários (${C})`})]})})]}),e.jsx(ne,{alignSelf:"flex-end",width:s?"95%":"100%",marginTop:o?"0rem":"0.5rem",height:s||!o?"0.05rem":".1rem",_light:{bg:"muted.800"},_dark:{bg:"muted.50"}})]})}const Jt=p.div`
  width: 90%;
  display: flex;
  flex-direction: column;
  gap: 0.8rem;

  padding: 10px;
  border-radius: 5px;
  background-color: ${i.colors.white};
  box-shadow: 0 0 10px ${i.colors.black+"25"};
`,Gt=p.div`
  width: 100%;
  display: flex;
  flex-direction: row;

  & > p {
    font-weight: 600;
    font-size: 0.8rem;
    color: ${i.colors.black};
  }
`,Kt=p.div`
  width: 100%;
  display: flex;
  flex-direction: column;
  gap: 0.6rem;

  & > p {
    text-align: left;
    font-weight: 400;
    font-size: 0.75rem;
    color: ${i.colors.black};
    line-height: 1.3;

    max-width: 40ch;
    overflow: hidden;
    text-overflow: ellipsis;
    white-space: nowrap;
  }
`,Yt=p.div`
  width: 100%;
  display: flex;
  flex-wrap: wrap;
  flex-direction: row;
  gap: 0.7rem;

  #post-badge-text {
    color: ${i.colors.text[50]};
    font-size: 0.7rem;
    line-height: 1.5;
    font-weight: 500;
  }
`;p.button`
  width: 100%;
  display: flex;
  flex-direction: row;
  align-items: center;
  justify-content: flex-start;
  gap: 0.5rem;
  transition: all 0.5s ease;

  svg {
    color: ${i.colors.primary[600]};
  }

  & > p {
    font-size: 1.2rem;
    font-weight: 600;
    color: ${i.colors.primary[600]};
  }

  &:hover {
    /* filter: brightness(50%); */

    & > p {
      text-shadow: 1px 1px 2px rgba(0, 0, 0, 0.5);
      color: ${i.colors.primary[400]};
    }

    & > svg {
      color: ${i.colors.primary[400]};
      filter: drop-shadow(1px 1px 2px rgba(0, 0, 0, 0.5));
    }
  }

  &:focus {
    box-shadow: none;
  }
`;function Wt(){return e.jsxs(Jt,{children:[e.jsxs(Gt,{children:[e.jsx(ae,{bg:"emerald.800",marginRight:"0.8rem",size:"xs",children:"SH"}),e.jsx("p",{children:"Sandalo Henrique - Horários de Atendimento coordenação de Eng. de Software"})]}),e.jsxs(Kt,{children:[e.jsxs("p",{children:["Prezad@s discentes,",e.jsx("br",{}),e.jsx("br",{}),"Prezad@s discentes, Bom dia! Espero que estejam bem. Deixo aqui informações sobre a dinâmica de atendimento da Coordenação de Eng. de Software durante o período da matrícula: - atendimento presencial nas 2a-feiras de 0830h às 0930h; - atendimento presencial nas 4a-feiras de 0830h às 1100h. Além do atendimento presencial, vocês têm acesso ao sistema Sigaa que disponibiliza canal de contato com o Coordenador para envio de mensagens.",e.jsx("br",{}),e.jsx("br",{}),"***Não haverá atendimento via Chat Teams durante esse período.",e.jsx("br",{}),"*** Att, TA",e.jsx("br",{}),e.jsx("br",{}),"Para visualizar acesse"," ",e.jsx("a",{href:"https://teste.com.br/teste",children:"https://teste.com.br/teste"}),e.jsx("br",{})]}),e.jsx(Yt,{children:["TCC","Eng. Software","Eng. Eletrônica","27/07"].map((t,o)=>e.jsx(Ne,{variant:"solid",bg:i.colors.tertiary[500],alignSelf:"center",size:"sm",textDecoration:"solid .8rem bold",children:e.jsx("p",{id:"post-badge-text",children:t})},o))})]})]})}const $t=p.div`
  display: flex;
  width: 100%;
  align-items: center;
  justify-content: center;
  height: max-content;
  gap: 1rem;

  @media (max-width: 768px) {
    height: 30vh;
    /* overflow-y: hidden; */
  }
`;function _({accessibilityLabel:t}){return e.jsxs($t,{children:[e.jsx(Xe,{size:"lg",color:i.colors.primary[700],accessibilityLabel:t}),e.jsx(He,{color:"primary.700",fontSize:"md",children:"Carregando..."})]})}function _t(){const{token:t}=J(),o=O(),s=H("(max-width: 768px)"),[r,n]=a.useState(!1),[c,d]=a.useState([]),[u,l]=a.useState([]),[h,m]=a.useState("-created_at"),[C,g]=a.useState(""),[x,w]=a.useState(""),[Q,S]=a.useState(!1);a.useEffect(()=>window.scrollTo(0,0),[]),a.useEffect(()=>console.log(c),[c]),a.useEffect(()=>{r||(S(!0),Tt({search:x,order_by:h,category__id__in:u}).then(b=>{d(b.data)}).finally(()=>S(!1)))},[r,x,h,u]);const k=a.useCallback(b=>{o("/create-topic")},[o]),A=a.useCallback(b=>{l(b.map(f=>f.value))},[]),U=a.useCallback(b=>{m(b.value)},[]),z=a.useCallback((b,f)=>{g(f)},[]),E=a.useCallback(()=>{w(C)},[C]);return e.jsx(e.Fragment,{children:e.jsxs(ct,{children:[e.jsxs(dt,{children:[e.jsx(Oe,{isDisabled:t,label:t?null:"Para criar um tópico é necessário estar logado",children:e.jsx(j,{isDisabled:!t,bgColor:i.colors.success[600],size:s?"xs":"lg",borderRadius:"4px",rightIcon:e.jsx(le,{}),onPress:()=>k(!0),children:e.jsx("p",{children:"Criar Tópico"})})}),e.jsx(vt,{onChangeCategoriesFilter:A,onChangeOrderBy:U,onChangeSearchInput:z,handleSearch:E}),Q?e.jsx(_,{accessibilityLabel:"Carregando os tópicos..."}):e.jsx(e.Fragment,{children:e.jsx(ut,{children:c.map(b=>e.jsx(ee,{id:b.id,title:b.title,rating:b.rating,files:b.files,currentRating:b.current_user_rating,content:b.content,author:b.author.name,commentsCount:b.comments_count,categories:b.categories,isSave:b.current_user_has_saved},b.id))})})]}),e.jsx(pt,{children:e.jsxs(ht,{children:[e.jsxs("div",{id:"favorite-title-container",children:[e.jsx("h1",{id:"title-favorites",children:"Tópicos Fixados"}),e.jsx(Je,{color:i.colors.rose[600],size:"22"})]}),Array.from({length:10}).map((b,f)=>e.jsx(Wt,{},f))]})})]})})}const eo="/assets/InlineLogo-bd32ea4c.png",to=p.header`
  width: 100%;
  height: 6rem;
  display: flex;
  align-items: center;
  justify-content: center;
  background: ${i.colors.primary[800]};

  .header-container {
    width: 80%;
    height: 100%;
    display: flex;
    flex-direction: row;
    align-items: center;
    justify-content: space-between;
    background: ${i.colors.primary[800]};

    & > button > img {
      width: 15rem;
    }
  }

  .user-container {
    display: flex;
    flex-direction: row;
    align-items: center;
    justify-content: center;
    gap: 1rem;
    color: ${i.colors.white};
  }

  @media (max-width: 768px) {
    .header-container {
      width: 90%;
      display: flex;
      align-items: center;
      /* justify-content: space-around; */

      & > button > img {
        width: 10rem;
      }
    }

    .user-container {
      /* display: none; */
    }

    .user-name {
      display: none;
    }
  }
`;p.button`
  width: 4rem;
  height: 4rem;
  display: flex;
  justify-content: center;
  align-items: center;
  background: ${i.colors.dark[200]};
  transition: all 0.2s;

  &:hover {
    transform: scale(1.05);
    box-shadow: 0 0 10px ${i.colors.tertiary[300]};
  }
`;function oo(){const{name:t,token:o}=J(),s=O(),r=H("(max-width: 768px)");function n(l){l.preventDefault(),s("/")}function c(l){l.preventDefault(),s("/login/logon")}function d(){return o?r?"":t:r?"":"Entrar"}function u(){return e.jsxs(e.Fragment,{children:[!o&&e.jsx(G.Item,{onPress:()=>s("/login/logon"),children:"Entrar"}),e.jsx(G.Item,{onPress:()=>s("/"),children:"Feed"}),o&&e.jsx(G.Item,{onPress:()=>s("/profile"),children:"Perfil"}),o&&e.jsx(G.Item,{onPress:()=>s("/saved-topics"),children:"Tópicos Salvos"}),e.jsx(G.Item,{onPress:()=>s("/categories"),children:"Categorias"})]})}return e.jsx(to,{children:e.jsxs("section",{className:"header-container",children:[e.jsx("button",{onClick:l=>n(l),children:e.jsx("img",{src:eo,alt:"Logo UnBFórum em linha"})}),e.jsx("div",{className:"user-container",children:o?e.jsx(G,{w:"190",placement:"bottom right",trigger:l=>e.jsx(j,{rightIcon:o?e.jsx(Ge,{size:"30",color:i.colors.tertiary[300]}):e.jsx(Ce,{size:30,color:i.colors.white}),...l,children:d()}),children:u()}):e.jsx(j,{variant:"solid",bgColor:i.colors.tertiary[500],_hover:{shadow:"8"},rightIcon:e.jsx(Ce,{size:30,color:i.colors.white}),onPress:l=>c(l),children:r?"":"Entrar"})})]})})}const so=p.div`
  background: ${i.colors.dark[900]};
  width: 100%;
  height: 100vh;
  display: flex;
  flex-direction: column;
  align-items: center;
  /* justify-content: center; */

  .content {
    width: 100%;
    display: flex;
    align-items: center;
    justify-content: center;
  }

  @media (max-width: 768px) {
    .content {
      width: 100%;
    }
  }
`;function $(){return e.jsxs(so,{children:[e.jsx(oo,{}),e.jsx("div",{className:"content",children:e.jsx(ke,{})})]})}const io=p.div`
  max-width: 65%;
  width: 65%;
  display: flex;
  flex-direction: column;
  align-items: center;
  justify-content: center;

  svg {
    width: 30rem;
    height: 25rem;
  }

  .buttons-container {
    width: 100%;
    display: flex;
    flex-direction: column;
    gap: 1.5rem;
  }

  @media (max-width: 768px) {
    max-width: 80%;

    svg {
      width: 20rem;
      height: 20rem;
    }
  }
`;function Y(){return e.jsxs("svg",{width:"404",height:"448",viewBox:"0 0 404 448",fill:"none",xmlns:"http://www.w3.org/2000/svg",xmlnsXlink:"http://www.w3.org/1999/xlink",children:[e.jsx("path",{fillRule:"evenodd",clipRule:"evenodd",d:"M174.519 157.247H250.075V115.315H244.666C243.959 115.315 243.267 115.175 242.604 114.909C241.94 114.643 241.363 114.265 240.858 113.774C240.353 113.284 239.964 112.723 239.69 112.079C239.416 111.435 239.271 110.762 239.271 110.076V101.796L226.967 113.844C225.885 114.839 224.587 115.329 223.087 115.315H174.519V157.247ZM163.73 146.767V162.487C163.73 163.187 163.859 163.845 164.134 164.49C164.408 165.134 164.797 165.709 165.302 166.199C165.807 166.689 166.398 167.068 167.062 167.334C167.711 167.6 168.403 167.726 169.124 167.726H228.482V188.699H179.914C178.414 188.671 177.159 189.203 176.135 190.268L163.73 202.218V193.939C163.73 193.238 163.585 192.566 163.311 191.935C163.037 191.291 162.648 190.716 162.143 190.226C161.638 189.736 161.047 189.357 160.398 189.091C159.734 188.825 159.042 188.699 158.32 188.699H152.926V146.767H163.73ZM163.73 136.274V110.076C163.744 109.375 163.888 108.717 164.162 108.086C164.436 107.442 164.826 106.881 165.331 106.391C165.836 105.901 166.413 105.522 167.076 105.256C167.725 104.99 168.403 104.85 169.124 104.836H220.923L240.901 85.4323C241.666 84.6617 242.589 84.1714 243.671 83.9612C244.767 83.7371 245.82 83.8492 246.83 84.2835C247.811 84.7178 248.59 85.3622 249.181 86.2308C249.758 87.1135 250.061 88.0661 250.075 89.1029V104.836H255.47C256.177 104.836 256.869 104.962 257.533 105.228C258.197 105.494 258.774 105.873 259.278 106.363C259.783 106.853 260.173 107.428 260.447 108.072C260.721 108.703 260.865 109.375 260.865 110.076V162.487C260.865 163.187 260.721 163.845 260.447 164.49C260.173 165.134 259.783 165.709 259.278 166.199C258.774 166.689 258.197 167.068 257.533 167.334C256.869 167.6 256.177 167.726 255.47 167.726H239.271V193.939C239.271 194.625 239.142 195.298 238.868 195.942C238.593 196.587 238.204 197.147 237.699 197.637C237.194 198.128 236.603 198.506 235.939 198.772C235.29 199.038 234.598 199.178 233.877 199.178H182.179L162.215 218.68C161.104 219.647 159.821 220.137 158.32 220.137C157.642 220.277 156.965 220.277 156.272 220.137C155.19 219.717 154.339 219.016 153.719 218.05C153.113 217.083 152.839 216.032 152.926 214.897V199.178H147.531C146.824 199.178 146.132 199.038 145.468 198.772C144.805 198.506 144.228 198.128 143.723 197.637C143.218 197.147 142.828 196.587 142.554 195.942C142.28 195.298 142.136 194.625 142.136 193.939V141.528C142.15 140.827 142.295 140.169 142.583 139.538C142.857 138.894 143.247 138.334 143.752 137.843C144.256 137.353 144.833 136.975 145.483 136.708C146.146 136.442 146.824 136.288 147.531 136.274H163.73Z",fill:"#164E63"}),e.jsx("mask",{id:"mask0_3637_13950",style:{maskType:"luminance"},maskUnits:"userSpaceOnUse",x:"45",y:"0",width:"313",height:"304",children:e.jsx("path",{d:"M45 0H358V304H45V0Z",fill:"white"})}),e.jsx("g",{mask:"url(#mask0_3637_13950)",children:e.jsx("path",{fillRule:"evenodd",clipRule:"evenodd",d:"M201.493 10.4794C200.065 10.4794 198.694 10.7456 197.367 11.2779C196.04 11.8103 194.886 12.5668 193.862 13.5615C192.852 14.5422 192.074 15.677 191.525 16.9519C190.977 18.2408 190.703 19.5718 190.703 20.9728C190.703 22.3597 190.977 23.6907 191.525 24.9796C192.074 26.2685 192.852 27.4033 193.862 28.384C194.886 29.3647 196.04 30.1212 197.367 30.6536C198.694 31.186 200.065 31.4521 201.493 31.4521C202.935 31.4521 204.306 31.186 205.633 30.6536C206.96 30.1212 208.114 29.3647 209.138 28.384C210.148 27.4033 210.926 26.2685 211.475 24.9796C212.023 23.6907 212.297 22.3597 212.297 20.9728C212.297 19.5718 212.023 18.2408 211.475 16.9519C210.926 15.677 210.148 14.5422 209.138 13.5615C208.114 12.5668 206.96 11.8103 205.633 11.2779C204.306 10.7456 202.935 10.4794 201.493 10.4794ZM175.052 62.9043H227.948C227.63 61.4332 227.197 59.9902 226.635 58.6033C226.087 57.2023 225.394 55.8573 224.601 54.5684C223.808 53.2935 222.899 52.0747 221.889 50.9539C220.88 49.8191 219.769 48.7683 218.572 47.8297C217.374 46.877 216.105 46.0364 214.763 45.2939C213.422 44.5514 212.023 43.9349 210.566 43.4306C209.109 42.9262 207.623 42.548 206.094 42.2958C204.58 42.0296 203.051 41.9035 201.493 41.9035C199.949 41.9035 198.42 42.0296 196.906 42.2958C195.377 42.548 193.891 42.9262 192.434 43.4306C190.977 43.9349 189.578 44.5514 188.237 45.2939C186.895 46.0364 185.626 46.877 184.428 47.8297C183.231 48.7683 182.121 49.8191 181.111 50.9539C180.101 52.0747 179.192 53.2935 178.399 54.5684C177.606 55.8573 176.913 57.2023 176.365 58.6033C175.803 59.9902 175.37 61.4332 175.052 62.9043ZM121.09 66.785C121.595 67.2754 121.984 67.8358 122.258 68.4802C122.518 69.1247 122.662 69.7831 122.662 70.4836C122.648 71.1841 122.518 71.8426 122.244 72.487C121.97 73.1315 121.58 73.6919 121.075 74.1822C120.556 74.6726 119.979 75.0508 119.316 75.317C118.667 75.5832 117.974 75.7093 117.253 75.7093C116.546 75.7093 115.854 75.5832 115.19 75.317C114.541 75.0508 113.95 74.6726 113.445 74.1822C112.94 73.6919 112.551 73.1315 112.276 72.487C112.002 71.8426 111.858 71.1841 111.858 70.4836C111.858 69.7831 111.988 69.1247 112.262 68.4802C112.536 67.8358 112.926 67.2754 113.43 66.785C113.935 66.2386 114.541 65.8043 115.234 65.5101C115.926 65.2159 116.662 65.0618 117.426 65.0618C118.176 65.0618 118.912 65.2159 119.604 65.5101C120.297 65.8043 120.902 66.2386 121.407 66.785H121.09ZM320.222 125.795C318.794 125.795 317.424 126.061 316.096 126.593C314.769 127.125 313.601 127.882 312.591 128.863C311.582 129.843 310.803 130.978 310.254 132.267C309.706 133.556 309.432 134.887 309.432 136.274C309.432 137.675 309.706 139.006 310.254 140.295C310.803 141.57 311.582 142.704 312.591 143.699C313.601 144.68 314.769 145.436 316.096 145.969C317.424 146.501 318.794 146.767 320.222 146.767C321.65 146.767 323.035 146.501 324.362 145.969C325.674 145.436 326.843 144.68 327.853 143.699C328.877 142.704 329.656 141.57 330.204 140.295C330.752 139.006 331.026 137.675 331.026 136.274C331.026 134.887 330.752 133.556 330.204 132.267C329.656 130.978 328.877 129.843 327.853 128.863C326.843 127.882 325.674 127.125 324.362 126.593C323.035 126.061 321.65 125.795 320.222 125.795ZM82.7781 125.795C81.3356 125.795 79.9653 126.061 78.6382 126.593C77.3256 127.125 76.1572 127.882 75.1475 128.863C74.1233 129.843 73.3444 130.978 72.7963 132.267C72.2481 133.556 71.9741 134.887 71.9741 136.274C71.9741 137.675 72.2481 139.006 72.7963 140.295C73.3444 141.57 74.1233 142.704 75.1475 143.699C76.1572 144.68 77.3256 145.436 78.6382 145.969C79.9653 146.501 81.3356 146.767 82.7781 146.767C84.2061 146.767 85.5765 146.501 86.9035 145.969C88.2306 145.436 89.399 144.68 90.4087 143.699C91.4185 142.704 92.1974 141.57 92.7455 140.295C93.2937 139.006 93.5677 137.675 93.5677 136.274C93.5677 134.887 93.2937 133.556 92.7455 132.267C92.1974 130.978 91.4185 129.843 90.4087 128.863C89.399 127.882 88.2306 127.125 86.9035 126.593C85.5765 126.061 84.2061 125.795 82.7781 125.795ZM293.782 178.205H346.662C346.359 176.734 345.927 175.305 345.364 173.904C344.801 172.517 344.124 171.173 343.33 169.884C342.537 168.609 341.628 167.39 340.618 166.255C339.609 165.12 338.498 164.084 337.301 163.131C336.103 162.192 334.834 161.338 333.493 160.609C332.151 159.867 330.752 159.25 329.295 158.746C327.838 158.241 326.352 157.863 324.823 157.597C323.309 157.345 321.765 157.219 320.222 157.219C318.678 157.219 317.149 157.345 315.62 157.597C314.106 157.863 312.62 158.241 311.163 158.746C309.706 159.25 308.307 159.867 306.966 160.609C305.61 161.338 304.34 162.192 303.143 163.131C301.96 164.084 300.85 165.12 299.84 166.255C298.83 167.39 297.921 168.609 297.128 169.884C296.32 171.173 295.642 172.517 295.08 173.904C294.517 175.305 294.084 176.734 293.782 178.205ZM56.3233 178.205H109.218C108.916 176.734 108.483 175.305 107.92 173.904C107.358 172.517 106.68 171.173 105.872 169.884C105.079 168.609 104.17 167.39 103.16 166.255C102.15 165.12 101.04 164.084 99.8569 163.131C98.6596 162.192 97.3902 161.338 96.0343 160.609C94.6928 159.867 93.2937 159.25 91.8368 158.746C90.3799 158.241 88.8941 157.863 87.3796 157.597C85.8505 157.345 84.3215 157.219 82.7781 157.219C81.2347 157.219 79.6912 157.345 78.1766 157.597C76.6476 157.863 75.1619 158.241 73.705 158.746C72.2481 159.25 70.8489 159.867 69.5074 160.609C68.166 161.338 66.8966 162.192 65.6993 163.131C64.5021 164.084 63.3914 165.12 62.3817 166.255C61.372 167.39 60.4632 168.609 59.6698 169.884C58.8765 171.173 58.1985 172.517 57.636 173.904C57.0734 175.305 56.6407 176.734 56.3233 178.205ZM201.493 241.11C200.065 241.11 198.694 241.376 197.367 241.908C196.04 242.441 194.886 243.197 193.862 244.178C192.852 245.159 192.074 246.293 191.525 247.582C190.977 248.871 190.703 250.202 190.703 251.589C190.703 252.976 190.977 254.321 191.525 255.61C192.074 256.885 192.852 258.02 193.862 259C194.886 259.981 196.04 260.752 197.367 261.27C198.694 261.802 200.065 262.068 201.493 262.068C202.935 262.068 204.306 261.802 205.633 261.27C206.96 260.752 208.114 259.981 209.138 259C210.148 258.02 210.926 256.885 211.475 255.61C212.023 254.321 212.297 252.976 212.297 251.589C212.297 250.202 212.023 248.871 211.475 247.582C210.926 246.293 210.148 245.159 209.138 244.178C208.114 243.197 206.96 242.441 205.633 241.908C204.306 241.376 202.935 241.11 201.493 241.11ZM175.052 293.521H227.948C227.63 292.05 227.197 290.621 226.635 289.22C226.087 287.819 225.394 286.488 224.601 285.199C223.808 283.91 222.899 282.705 221.889 281.57C220.88 280.435 219.769 279.399 218.572 278.446C217.374 277.493 216.105 276.653 214.763 275.91C213.422 275.182 212.023 274.565 210.566 274.061C209.109 273.557 207.623 273.164 206.094 272.912C204.58 272.66 203.051 272.534 201.493 272.534C199.949 272.534 198.42 272.66 196.906 272.912C195.377 273.164 193.891 273.557 192.434 274.061C190.977 274.565 189.578 275.182 188.237 275.91C186.895 276.653 185.626 277.493 184.428 278.446C183.231 279.399 182.121 280.435 181.111 281.57C180.101 282.705 179.192 283.91 178.399 285.199C177.606 286.488 176.913 287.819 176.365 289.22C175.803 290.621 175.37 292.05 175.052 293.521ZM231.611 45.9103C234.121 49.1046 236.025 52.607 237.338 56.4177C238.651 60.2144 239.3 64.1231 239.271 68.144C239.271 68.8305 239.141 69.5029 238.867 70.1474C238.593 70.7918 238.203 71.3522 237.699 71.8426C237.194 72.3329 236.602 72.7112 235.939 72.9774C235.29 73.2436 234.597 73.3837 233.876 73.3837H169.124C168.403 73.3837 167.71 73.2436 167.061 72.9774C166.398 72.7112 165.806 72.3329 165.301 71.8426C164.797 71.3522 164.407 70.7918 164.133 70.1474C163.859 69.5029 163.729 68.8305 163.729 68.144C163.715 64.0951 164.378 60.1583 165.734 56.3337C167.09 52.509 169.037 49.0065 171.605 45.8122C159.272 49.1186 147.833 54.2882 137.274 61.3212C136.38 61.9656 135.37 62.2738 134.26 62.2738C132.37 62.2598 130.899 61.4893 129.831 59.9622C129.427 59.4018 129.139 58.7714 128.995 58.0989C128.836 57.4264 128.836 56.7399 128.966 56.0535C129.096 55.381 129.355 54.7505 129.759 54.1761C130.149 53.5877 130.639 53.1114 131.23 52.7331C147.098 42.0576 164.58 35.3749 183.693 32.713C181.197 29.1685 179.942 25.2598 179.914 20.9728C179.914 19.5858 180.058 18.2268 180.332 16.8819C180.606 15.5229 181.01 14.22 181.558 12.9451C182.092 11.6702 182.77 10.4654 183.549 9.31656C184.342 8.18176 185.236 7.11701 186.232 6.13632C187.241 5.16964 188.323 4.30103 189.506 3.53049C190.689 2.77395 191.929 2.1295 193.242 1.59713C194.555 1.06475 195.896 0.672473 197.295 0.406285C198.68 0.1401 200.079 0 201.493 0C202.921 0 204.32 0.1401 205.705 0.406285C207.104 0.672473 208.445 1.06475 209.758 1.59713C211.071 2.1295 212.311 2.77395 213.494 3.53049C214.677 4.30103 215.759 5.16964 216.768 6.13632C217.764 7.11701 218.658 8.18176 219.451 9.31656C220.23 10.4654 220.894 11.6702 221.442 12.9451C221.99 14.22 222.394 15.5229 222.668 16.8819C222.942 18.2268 223.086 19.5858 223.086 20.9728C223.101 25.2458 221.875 29.1545 219.423 32.713C225.164 33.5116 230.818 34.7024 236.386 36.2715C241.954 37.8406 247.378 39.788 252.671 42.0996C257.965 44.3973 263.072 47.0591 267.976 50.0572C272.88 53.0553 277.554 56.3757 281.997 60.0042C286.425 63.6328 290.579 67.5556 294.445 71.7445C298.311 75.9334 301.874 80.3746 305.105 85.0539C308.336 89.7332 311.221 94.6086 313.745 99.6662C316.284 104.738 318.448 109.949 320.222 115.315C321.65 115.315 323.049 115.455 324.434 115.721C325.833 115.988 327.175 116.38 328.487 116.912C329.8 117.431 331.04 118.089 332.223 118.846C333.392 119.616 334.488 120.485 335.483 121.451C336.493 122.432 337.387 123.483 338.181 124.632C338.96 125.781 339.623 126.985 340.171 128.26C340.705 129.535 341.123 130.838 341.397 132.183C341.671 133.542 341.816 134.901 341.816 136.274C341.801 141.612 339.969 146.249 336.305 150.228C339.536 151.713 342.494 153.618 345.177 155.93C347.845 158.241 350.139 160.861 352.057 163.803C353.961 166.745 355.418 169.884 356.399 173.232C357.394 176.566 357.899 179.971 357.899 183.459C357.899 184.146 357.755 184.818 357.481 185.463C357.207 186.107 356.817 186.667 356.312 187.158C355.807 187.648 355.23 188.026 354.567 188.293C353.903 188.559 353.211 188.699 352.504 188.699H320.222C318.563 193.826 316.544 198.814 314.178 203.675C311.827 208.537 309.144 213.23 306.129 217.741C303.129 222.252 299.825 226.553 296.219 230.63C292.613 234.721 288.747 238.546 284.622 242.118C280.482 245.691 276.126 248.997 271.539 251.995C266.952 255.008 262.177 257.697 257.215 260.093C252.253 262.475 247.161 264.52 241.911 266.243C236.66 267.967 231.323 269.34 225.899 270.362C230.198 273.893 233.515 278.138 235.852 283.111C238.189 288.085 239.343 293.296 239.271 298.76C239.271 299.461 239.141 300.133 238.867 300.778C238.593 301.408 238.203 301.983 237.699 302.473C237.194 302.963 236.602 303.342 235.939 303.608C235.29 303.874 234.597 304 233.876 304H169.124C168.403 304 167.71 303.874 167.061 303.608C166.398 303.342 165.806 302.963 165.301 302.473C164.797 301.983 164.407 301.408 164.133 300.778C163.859 300.133 163.729 299.461 163.729 298.76C163.744 293.254 164.97 288.015 167.407 283.041C169.845 278.068 173.249 273.837 177.649 270.362C172.211 269.34 166.874 267.967 161.623 266.243C156.387 264.52 151.281 262.475 146.319 260.093C141.357 257.697 136.582 255.008 131.995 251.995C127.408 248.997 123.052 245.705 118.926 242.118C114.786 238.546 110.921 234.721 107.314 230.63C103.723 226.553 100.419 222.252 97.4047 217.741C94.4044 213.23 91.7069 208.537 89.3557 203.675C86.9901 198.814 84.9851 193.826 83.3118 188.699H50.3948C49.6736 188.699 48.9956 188.559 48.3321 188.293C47.6686 188.026 47.0771 187.648 46.5723 187.158C46.0674 186.667 45.678 186.107 45.4039 185.463C45.1298 184.818 45 184.146 45 183.459C45.0144 179.985 45.5193 176.566 46.5146 173.232C47.5099 169.898 48.9668 166.759 50.8708 163.831C52.7893 160.889 55.0684 158.255 57.7369 155.944C60.4199 153.632 63.3626 151.727 66.5792 150.228C65.1945 148.715 64.0549 147.034 63.175 145.212C62.2807 143.377 61.6893 141.458 61.372 139.454C61.0546 137.451 61.0402 135.447 61.3143 133.444C61.6027 131.441 62.1653 129.521 63.0308 127.672C63.8818 125.823 64.9925 124.127 66.3484 122.6C67.7044 121.059 69.2622 119.756 71.022 118.65C72.7674 117.557 74.6426 116.73 76.6476 116.156C78.6527 115.595 80.6865 115.301 82.7781 115.315C86.5285 104.079 91.8512 93.5999 98.7462 83.863C99.1645 83.3026 99.6838 82.8403 100.29 82.4761C100.895 82.1118 101.559 81.8596 102.266 81.7475C102.973 81.6355 103.665 81.6495 104.372 81.7896C105.064 81.9437 105.713 82.2099 106.305 82.6021C106.896 82.9944 107.372 83.4848 107.762 84.0732C108.137 84.6476 108.396 85.278 108.512 85.9645C108.641 86.637 108.627 87.3235 108.454 87.9959C108.295 88.6684 108.007 89.2849 107.603 89.8452C101.516 98.6154 96.8421 108.058 93.5677 118.145C95.2121 119.07 96.6979 120.177 98.0394 121.466C99.3664 122.768 100.52 124.211 101.458 125.809C102.41 127.406 103.131 129.087 103.622 130.866C104.112 132.631 104.357 134.439 104.357 136.274C104.343 141.612 102.511 146.249 98.8616 150.228C102.093 151.713 105.064 153.604 107.747 155.916C110.43 158.213 112.738 160.847 114.657 163.789C116.575 166.731 118.046 169.87 119.042 173.218C120.051 176.552 120.542 179.971 120.556 183.459C120.556 184.146 120.412 184.818 120.138 185.463C119.864 186.107 119.474 186.667 118.969 187.158C118.465 187.648 117.873 188.026 117.224 188.293C116.561 188.559 115.868 188.699 115.147 188.699H94.6496C96.294 193.28 98.2557 197.749 100.52 202.092C102.785 206.421 105.338 210.596 108.165 214.603C111.007 218.61 114.108 222.421 117.469 226.021C120.83 229.636 124.408 233.012 128.23 236.15C132.053 239.288 136.077 242.174 140.289 244.794C144.501 247.414 148.886 249.754 153.416 251.813C157.959 253.873 162.618 255.624 167.393 257.081C172.168 258.538 177.029 259.673 181.962 260.499C180.592 257.683 179.914 254.713 179.914 251.589C179.928 250.034 180.13 248.493 180.491 246.98C180.866 245.453 181.399 243.996 182.106 242.595C182.813 241.194 183.678 239.877 184.688 238.672C185.683 237.453 186.823 236.36 188.078 235.394C189.347 234.413 190.689 233.6 192.146 232.914C193.588 232.241 195.103 231.723 196.661 231.373C198.233 231.023 199.82 230.855 201.421 230.84C203.036 230.84 204.623 231.009 206.181 231.345C207.753 231.681 209.268 232.185 210.725 232.858C212.167 233.516 213.537 234.343 214.792 235.296C216.062 236.262 217.201 237.341 218.225 238.56C219.235 239.765 220.101 241.068 220.822 242.469C221.543 243.856 222.091 245.313 222.466 246.826C222.856 248.353 223.058 249.88 223.086 251.449C223.115 253.004 222.957 254.545 222.639 256.086C222.308 257.613 221.803 259.084 221.139 260.499C226.072 259.673 230.933 258.538 235.708 257.081C240.483 255.624 245.142 253.859 249.685 251.799C254.215 249.754 258.585 247.414 262.812 244.78C267.024 242.16 271.034 239.274 274.856 236.136C278.679 232.998 282.271 229.622 285.632 226.021C288.993 222.407 292.094 218.596 294.921 214.603C297.763 210.596 300.316 206.421 302.581 202.078C304.845 197.749 306.807 193.28 308.466 188.699H287.853C287.132 188.699 286.439 188.559 285.776 188.293C285.112 188.026 284.535 187.648 284.031 187.158C283.526 186.667 283.136 186.107 282.862 185.463C282.588 184.818 282.444 184.146 282.444 183.459C282.458 179.985 282.963 176.566 283.958 173.232C284.968 169.898 286.411 166.759 288.329 163.831C290.233 160.889 292.527 158.255 295.195 155.944C297.864 153.632 300.806 151.727 304.037 150.228C302.97 149.051 302.061 147.79 301.282 146.431C300.518 145.072 299.898 143.643 299.45 142.144C299.018 140.645 298.744 139.118 298.643 137.577C298.556 136.022 298.628 134.481 298.888 132.94C299.148 131.398 299.58 129.913 300.172 128.47C300.778 127.027 301.528 125.668 302.451 124.394C303.36 123.105 304.398 121.942 305.581 120.891C306.764 119.84 308.048 118.916 309.432 118.145C307.975 113.83 306.259 109.613 304.268 105.508C302.278 101.389 300.027 97.4246 297.532 93.5719C295.037 89.7332 292.31 86.0486 289.353 82.5321C286.396 79.0156 283.223 75.6953 279.847 72.5571C276.458 69.4189 272.895 66.4908 269.144 63.7869C265.394 61.069 261.485 58.5892 257.417 56.3477C253.349 54.1061 249.166 52.1027 244.853 50.3655C240.526 48.6142 236.112 47.1292 231.611 45.9103Z",fill:"#164E63"})}),e.jsxs("mask",{id:"mask1_3637_13950",style:{maskType:"luminance"},maskUnits:"userSpaceOnUse",x:"0",y:"315",width:"404",height:"133",children:[e.jsx("path",{d:"M0 315H403.886V447.631H0V315Z",fill:"white"}),e.jsx("path",{d:"M0 315H403.886V447.631H0V315Z",stroke:"white"})]}),e.jsxs("g",{mask:"url(#mask1_3637_13950)",children:[e.jsxs("mask",{id:"mask2_3637_13950",style:{maskType:"luminance"},maskUnits:"userSpaceOnUse",x:"0",y:"315",width:"404",height:"133",children:[e.jsx("rect",{x:"0.745361",y:"315.404",width:"403.144",height:"132.375",fill:"url(#pattern0)"}),e.jsx("rect",{x:"0.745361",y:"315.404",width:"403.144",height:"132.375",stroke:"black"})]}),e.jsxs("g",{mask:"url(#mask2_3637_13950)",children:[e.jsx("rect",{x:"0.745361",y:"315.404",width:"403.144",height:"132.375",fill:"url(#paint0_linear_3637_13950)"}),e.jsx("rect",{x:"0.745361",y:"315.404",width:"403.144",height:"132.375",stroke:"black"})]})]}),e.jsxs("defs",{children:[e.jsx("pattern",{id:"pattern0",patternContentUnits:"objectBoundingBox",width:"1",height:"1",children:e.jsx("use",{xlinkHref:"#image0_3637_13950",transform:"scale(0.00102041 0.00309598)"})}),e.jsxs("linearGradient",{id:"paint0_linear_3637_13950",x1:"202.317",y1:"315.404",x2:"202.317",y2:"447.779",gradientUnits:"userSpaceOnUse",children:[e.jsx("stop",{offset:"0.582581",stopColor:"#164E63"}),e.jsx("stop",{offset:"1",stopColor:"white",stopOpacity:"0"})]}),e.jsx("image",{id:"image0_3637_13950",width:"980",height:"323",xlinkHref:"data:image/png;base64,"})]})]})}function no(){const t=O();function o(n){n.preventDefault(),t("/login/create-account")}function s(n){n.preventDefault(),t("/login")}function r(n){n.preventDefault(),t("/")}return e.jsxs(io,{children:[e.jsx(Y,{}),e.jsxs("div",{className:"buttons-container",children:[e.jsx(j,{onPress:n=>s(n),variant:"outline",size:"lg",children:"Entrar"}),e.jsx(j,{onPress:n=>o(n),variant:"solid",size:"lg",children:"Criar Conta"}),e.jsx(j,{onPress:n=>r(n),variant:"link",size:"lg",children:"Ir para o UnBFórum"})]})]})}const ro=p.div`
  max-width: 65%;
  width: 65%;
  display: flex;
  flex-direction: column;
  align-items: center;
  justify-content: center;

  & > svg {
    width: 10rem;
    height: 12rem;
  }

  .buttons-container {
    width: 100%;
    display: flex;
    flex-direction: column;
    gap: 1rem;
  }

  @media (max-width: 768px) {
    max-width: 90%;
    width: 90%;
  }
`,ao=t=>{let o=!0;const s=[],{email:r,password:n}=t;return(!r||r==="")&&(o=!1,s.push("Email Institucional")),(!n||n==="")&&(o=!1,s.push("Senha")),{isValid:o,fieldErrors:s}},lo=async t=>{const{username:o,password:s}=t,r=new FormData;return r.append("username",o),r.append("password",s),await N.post("auth/login",r)},co=t=>{const{email:o,password:s}=t;return{username:o,password:s}};function uo(){const t=O(),o=Z(),{changeToken:s}=J(),[r,n]=a.useState({email:"",password:""}),[c,d]=a.useState(!1),u=a.useCallback((h,m)=>{n({...r,[h]:m})},[r]);function l(h){h.preventDefault();const{isValid:m,fieldErrors:C}=ao(r);if(!m){o.show({placement:"top-right",render:()=>e.jsx(v,{id:"login-user-error",title:"Campos Inválidos",description:`Os sequintes campos estão incorretos: ${C.reduce((g,x,w)=>w===0?`'${x}'`:w===C.length-1?`${g} e '${x}'`:`${g}, '${x}'`,"")}`,status:""})});return}d(!0),lo(co(r)).then(g=>{o.show({placement:"top-right",render:()=>e.jsx(v,{id:"create-user-success",title:"Bem-vindo ao UnBFórum",description:`Usuário logado com sucesso!
Agora você pode criar tópicos e interagir com eles!`,status:"success"})}),s(g.data.access_token),t("/")}).catch(g=>{o.show({placement:"top-right",render:()=>{let x="";return typeof g.response.data.detail=="object"?x=g.response.data.detail[0].msg.split(", ")[1]:x=g.response.data.detail,e.jsx(v,{id:"create-user-error",title:"Campos Inválidos",description:`Erro: ${x}`,status:""})}})}).finally(()=>{d(!1)})}return e.jsxs(ro,{children:[e.jsx(Y,{}),e.jsxs("div",{className:"buttons-container",children:[e.jsx(V,{name:"email",onChange:u,accessibilityLabel:"Digite o seu email Institucional",inputType:"text",label:"Email Institucional",placeholder:"Digite o email 240014099@unb.br..."}),e.jsx(V,{name:"password",onChange:u,accessibilityLabel:"Senha",inputType:"password",label:"Digite a sua senha",placeholder:"Digite a sua senha"}),e.jsx(j,{onPress:h=>l(h),variant:"solid",size:"lg",isLoading:c,isLoadingText:"Entrando...",children:"Entrar"}),e.jsx(j,{onPress:h=>t(-1),variant:"outline",size:"lg",children:"Voltar"})]})]})}const po=p.div`
  width: 100%;
  height: 100vh;
  display: flex;
  /* align-items: center; */
  /* justify-content: space-between; */

  .create-account-image {
    object-fit: cover;
    object-position: center;
    width: 50%;
    height: 100vh;
  }

  .content {
    width: 50%;
    display: flex;
    align-items: center;
    justify-content: center;
  }

  @media (max-width: 768px) {
    .create-account-image {
      display: none;
    }

    .content {
      width: 100%;
    }
  }
`,ho="/assets/create-account-24b27e13.png";function mo(){return e.jsxs(po,{children:[e.jsx("img",{className:"create-account-image",src:ho,alt:""}),e.jsx("div",{className:"content",children:e.jsx(ke,{})})]})}const go=p.div`
  max-width: 65%;
  width: 65%;
  display: flex;
  flex-direction: column;
  align-items: center;
  justify-content: center;

  & > svg {
    width: 10rem;
    height: 12rem;
  }

  .inputs-container {
    width: 100%;
    display: flex;
    flex-direction: column;
    gap: 1rem;
  }

  @media (max-width: 768px) {
    max-width: 90%;
    width: 90%;
    height: 100vh;

    padding: 1rem 0 1rem 0;

    .inputs-container {
      gap: 0.7rem;
    }
  }
`,xo=[{label:"Aluno",value:"Aluno"},{label:"Coordenador",value:"Coordenador"},{label:"Professor",value:"Professor"},{label:"Secretaria",value:"Secretaria"},{label:"Técnico",value:"Técnico"}],fo=[{label:"Engenharia Aeroespacial",value:"Engenharia Aeroespacial"},{label:"Engenharia Automotiva",value:"Engenharia Automotiva"},{label:"Engenharia de Energia",value:"Engenharia de Energia"},{label:"Engenharia de Software",value:"Engenharia de Software"},{label:"Engenharia Eletrônica",value:"Engenharia Eletrônica"},{label:"Engenharias",value:"Engenharias"}],Co=[{input:"normal",type:"text",name:"name",label:"Nome Completo",placeholder:"Digite o nome completo...",accessibilityLabel:"Digite o nome completo",options:[]},{input:"normal",type:"text",name:"email",label:"Email Institucional",placeholder:"Digite o email 240014099@unb.br...",accessibilityLabel:"Digite o email",options:[]},{input:"select",type:"text",name:"profile",label:"Papel na Instituição",placeholder:"Selecione o seu papel...",accessibilityLabel:"Selecione o seu papel na instituição",options:xo},{input:"select",type:"text",name:"course",label:"Curso",placeholder:"Selecione o seu curso...",accessibilityLabel:"Selecione o seu curso",options:fo},{input:"normal",type:"password",name:"password",label:"Senha",placeholder:"Digite a sua senha...",accessibilityLabel:"Digite a sua senha",options:[]},{input:"normal",type:"password",name:"passwordConfirm",label:"Confirme a senha",placeholder:"Digite a sua senha novamente...",accessibilityLabel:"Digite a sua senha novamente",options:[]}],wo=p.div`
  width: 100%;
  display: flex;
  flex-direction: column;
  /* gap: 0.3rem; */

  .create-account-label {
    display: flex;
    flex-direction: row;
    gap: 0.3rem;

    font-size: ${({fontSize:t})=>t};
    line-height: 1.5rem;
    color: ${i.colors.text[500]};

    #required {
      color: ${i.colors.danger[600]};
    }
  }

  svg {
    width: 1.5rem;
  }

  option {
    background-color: ${i.colors.gray[200]};
    color: ${i.colors.black};
    font-family: 'Poppins', 'Montserrat', sans-serif;
    font-size: 0.9rem;
    gap: 0.3rem;
  }

  @media (max-width: 768px) {
    .create-account-label {
      font-size: ${({fontSize:t})=>Number(t.split("rem")[0])-.2+"rem"};
    }
  }
`;function bo({label:t,placeholder:o,options:s=[],onChange:r,isRequired:n=!0,fontSize:c="0.9rem",isEditing:d=!0,name:u}){return e.jsxs(wo,{fontSize:c,children:[e.jsxs("label",{className:"create-account-label",children:[t,n&&e.jsx("p",{id:"required",children:"*"})]}),e.jsx(we,{variant:"outline",height:9,size:"md",isDisabled:!d,onValueChange:l=>r(u,l),placeholder:o,accessibilityLabel:"Selecione a opção",children:s.map(l=>e.jsx(we.Item,{label:l.label,value:l.value},l.label))})]})}const jo=async t=>await N.post("users",t),je=async()=>await N.get("users/me"),yo=async t=>await N.patch("users/me",t),vo=async t=>await N.post("users/me/change_password",t),No=async()=>await N.delete("users/me"),ko=t=>{let o=!0;const s=[],{name:r,email:n,profile:c,course:d,password:u,passwordConfirm:l}=t;return(!r||r==="")&&(o=!1,s.push("Nome Completo")),(!n||n==="")&&(o=!1,s.push("Email Institucional")),(!c||c==="")&&(o=!1,s.push("Papel na Instituição")),(!d||d==="")&&(o=!1,s.push("Curso")),(!u||u==="")&&(o=!1,s.push("Senha")),(!l||l===""||l!==u)&&(o=!1,s.push("Confirme a senha")),{isValid:o,fieldErrors:s}},Do=t=>{const{name:o,email:s,profile:r,course:n,password:c}=t;return{name:o,email:s,password:c,tags:[r,n]}};function So(){const t=O(),o=Z(),s=ce({lg:"lg",base:"md"}),[r,n]=a.useState({name:"",email:"",profile:"",course:"",password:"",passwordConfirm:""}),[c,d]=a.useState(!1),u=a.useCallback((m,C)=>{n({...r,[m]:C})},[r]),l=async m=>{m.preventDefault();const{isValid:C,fieldErrors:g}=ko(r);if(console.log(r),!C){o.show({placement:"top-right",render:()=>e.jsx(v,{id:"create-user-error",title:"Campos Inválidos",description:`Os sequintes campos estão incorretos: ${g.reduce((x,w,Q)=>Q===0?`'${w}'`:Q===g.length-1?`${x} e '${w}'`:`${x}, '${w}'`,"")}`,status:""})});return}d(!0),jo(Do(r)).then(x=>{o.show({placement:"top-right",render:()=>e.jsx(v,{id:"create-user-success",title:"Estamos quase lá!",description:`Conta criada com sucesso!
Clique em 'Entrar' e faça o seu login na aplicação!`,status:"success"})}),t("/login/logon")}).catch(x=>{o.show({placement:"top-right",render:()=>{let w="";return typeof x.response.data.detail=="object"?w=x.response.data.detail[0].msg.split(", ")[1]:w=x.response.data.detail,e.jsx(v,{id:"create-user-error",title:"Campos Inválidos",description:`Erro: ${w}`,status:""})}})}).finally(()=>{d(!1)})};function h(m){switch(m.input){case"select":return e.jsx(bo,{label:m.label,placeholder:m.placeholder,options:m.options,name:m.name,onChange:u,accessibilityLabel:m.accessibilityLabel},m.label);default:return e.jsx(V,{label:m.label,placeholder:m.placeholder,inputType:m.type,name:m.name,onChange:u,accessibilityLabel:m.accessibilityLabel},m.label)}}return e.jsxs(go,{children:[e.jsx(Y,{}),e.jsxs("div",{className:"inputs-container",children:[Co.map(m=>h(m)),e.jsx(j,{onPress:m=>l(m),variant:"solid",size:s,isLoading:c,isLoadingText:"Criando usuário...",children:"Criar Conta"}),e.jsx(j,{onPress:m=>t(-1),variant:"outline",size:s,children:"Voltar"})]})]})}const Qo=p.section`
  width: 80%;
  height: calc(100vh - 6rem);
  display: flex;
  align-items: center;
  justify-content: space-between;

  .logo-profile-container {
    width: 30%;

    svg {
      width: 17rem;
      height: 19rem;
    }
  }

  .inputs-profile-container {
    width: 60%;
    display: flex;
    flex-direction: column;
    align-items: center;
    justify-content: center;
    gap: 1rem;

    & > div[role='button'] {
      width: 100%;
    }

    .profile-page-title {
      font-weight: bold;
      font-size: 2rem;
    }
  }

  .sub-buttons-container {
    width: 100%;
    display: flex;
    align-items: center;
    justify-content: space-between;

    & > div[role='button'] {
      width: 45%;
    }
  }

  @media (max-width: 930px) {
    .logo-profile-container {
      svg {
        width: 15rem;
        height: 13rem;
      }
    }
  }

  @media (max-width: 768px) {
    .logo-profile-container {
      display: none;
    }

    .inputs-profile-container {
      width: 100%;
    }
  }
`,zo=t=>{let o=!0;const s=[];return(!t||t==="")&&(o=!1,s.push("Nome Completo")),{isValid:o,fieldErrors:s}},Ao=(t,o)=>{let s=!0;const r=[];let n="";return(!t||t==="")&&(s=!1,r.push("Senha Atual")),(!o||o==="")&&(s=!1,r.push("Nova Senha")),!o&&o===t&&(s=!1,n="Nova senha não pode ser a mesma que a atual"),{isValid:s,fieldErrors:r,specialError:n}},Eo=p.div`
  position: fixed;
  top: 0;
  left: 0;
  width: 100%;
  height: 100%;
  background-color: rgba(0, 0, 0, 0.5); /* Fundo sombreado */
  display: flex;
  align-items: center;
  justify-content: center;
`,Lo=p.div`
  width: 40%;
  display: flex;
  align-items: flex-start;
  justify-content: center;
  flex-direction: column;
  gap: 1rem;

  position: relative;
  background-color: white;
  padding: 20px 30px;
  border-radius: 8px;
  box-shadow: 0 0 10px rgba(0, 0, 0, 0.3);

  #buttons-dialog-container {
    width: 100%;
    display: flex;
    flex-direction: row;
    align-items: center;
    justify-content: flex-end;
    gap: 1rem;
  }

  & > p {
    margin-top: 1rem;
    font-size: 1rem;
    font-weight: 400;
    color: ${i.colors.text[800]};
  }

  @media (max-width: 768px) {
    width: 80%;

    padding: 20px 20px;

    #buttons-dialog-container {
      gap: 0.5rem;
    }

    & > p {
      margin-top: 1.5rem;
      font-size: 0.9rem;
    }
  }
`;p.span`
  position: absolute;
  top: 10px;
  right: 10px;
  cursor: pointer;
`;const To=p.button`
  position: absolute;
  top: 0;
  right: 0;

  display: flex;
  align-items: center;
  justify-content: center;

  padding: 10px;
  border-radius: 0 8px 0 0;
  transition: 0.3s all ease;

  background-color: ${i.colors.danger[600]};

  &:hover {
    filter: brightness(70%);
  }

  @media (max-width: 768px) {
    padding: 0.5rem;
  }
`;function ye({question:t,handleAccept:o,handleClose:s,cancelText:r="Cancelar",acceptText:n,isOpen:c,isLoadingAcceptButton:d=!1,loadingAcceptButtonText:u=""}){const l=ce({lg:"md",base:"sm"});return e.jsx(e.Fragment,{children:c&&e.jsx(Eo,{children:e.jsxs(Lo,{children:[e.jsx(To,{onClick:s,children:e.jsx(re,{size:"21",color:i.colors.primary[50]})}),e.jsx("p",{children:t}),e.jsxs("div",{id:"buttons-dialog-container",children:[e.jsx(j,{size:l,variant:"outline",onPress:s,children:r}),e.jsx(j,{size:l,onPress:o,bgColor:i.colors.danger[500],isLoading:d,isLoadingText:u,children:n})]})]})})})}function Io(){const t=O(),o=Z(),{name:s,email:r,setName:n,logoutUser:c}=J(),d=ce({lg:"lg",base:"md"}),[u,l]=a.useState(!1),[h,m]=a.useState(!1),[C,g]=a.useState(!1),[x,w]=a.useState(!1),[Q,S]=a.useState({email:r,id:0,name:s,role:"",tags:[]}),[k,A]=a.useState(!1),[U,z]=a.useState(!1),[E,b]=a.useState(!1),[f,y]=a.useState(""),[T,L]=a.useState("");a.useEffect(()=>{je().then(I=>{S(I.data),n(I.data.name)})},[n]);const F=a.useCallback((I,B)=>{S({...Q,[I]:B})},[Q]),D=a.useCallback(()=>{z(!0),No().then(()=>{}).finally(()=>{o.show({placement:"top-right",render:()=>e.jsx(v,{id:"delete-user-success",title:"Usuário deletado!",description:"A qualquer momento você poderá criar uma nova conta",status:"success"})}),c(),z(!1),m(!1),t("/")})},[t,c,o]),M=I=>{I.preventDefault();const{fieldErrors:B,isValid:te}=zo(Q.name);if(!te){o.show({placement:"top-right",render:()=>e.jsx(v,{id:"create-user-error",title:"Campos Inválidos",description:`Os sequintes campos estão incorretos: ${B.reduce((X,P,R)=>R===0?`'${P}'`:R===B.length-1?`${X} e '${P}'`:`${X}, '${P}'`,"")}`,status:""})});return}A(!0),yo({name:Q.name}).then(X=>{o.show({placement:"top-right",render:()=>e.jsx(v,{id:"update-user-success",title:"Usuário atualizado!",description:"",status:"success"})}),n(Q.name)}).catch(X=>{o.show({placement:"top-right",render:()=>{let P="";return typeof X.response.data.detail=="object"?P=X.response.data.detail[0].msg.split(", ")[1]:P=X.response.data.detail,e.jsx(v,{id:"create-user-error",title:"Campos Inválidos",description:`Erro: ${P}`,status:""})}})}).finally(()=>{A(!1),g(!1)})},W=I=>{I.preventDefault();const{fieldErrors:B,isValid:te,specialError:X}=Ao(f,T);if(!te){o.show({placement:"top-right",render:()=>e.jsx(v,{id:"create-user-error",title:"Campos Inválidos",description:B.length>0?`Os sequintes campos estão incorretos: ${B.reduce((P,R,ue)=>ue===0?`'${R}'`:ue===B.length-1?`${P} e '${R}'`:`${P}, '${R}'`,"")}`:X,status:""})});return}b(!0),vo({current_password:f,new_password:T}).then(P=>{o.show({placement:"top-right",render:()=>e.jsx(v,{id:"create-user-success",title:"Senha atualizada com sucesso!",description:"",status:"success"})})}).catch(P=>{o.show({placement:"top-right",render:()=>{let R="";return typeof P.response.data.detail=="object"?R=P.response.data.detail[0].msg.split(", ")[1]:R=P.response.data.detail,e.jsx(v,{id:"create-user-error",title:"Campos Inválidos",description:`Erro: ${R}`,status:""})}})}).finally(()=>{b(!1),w(!1),y(""),L("")})},Ee=()=>C?e.jsxs(e.Fragment,{children:[e.jsx(j,{variant:"solid",size:d,onPress:()=>{g(!1),je().then(I=>{S(I.data)})},children:"Cancelar"}),e.jsx(j,{variant:"outline",size:d,isLoading:k,isLoadingText:"Atualizando Usuário...",onPress:I=>M(I),children:"Atualizar Usuário"})]}):x?e.jsxs(e.Fragment,{children:[e.jsx(j,{variant:"solid",size:d,onPress:()=>{w(!1),y(""),L("")},children:"Cancelar"}),e.jsx(j,{variant:"outline",size:d,isLoading:E,isLoadingText:"Atualizando Senha...",onPress:I=>W(I),children:"Alterar Senha"})]}):e.jsxs(e.Fragment,{children:[e.jsx(j,{variant:"solid",size:d,onPress:()=>g(!0),children:"Editar Perfil"}),e.jsx(j,{variant:"outline",size:d,onPress:()=>w(!0),children:"Alterar Senha"})]});return e.jsxs(Qo,{children:[e.jsx("div",{className:"logo-profile-container",children:e.jsx(Y,{})}),e.jsxs("section",{className:"inputs-profile-container",children:[e.jsx("h1",{className:"profile-page-title",children:"Perfil de Usuário"}),e.jsx(V,{name:"name",onChange:F,isEditing:C,value:Q.name,label:"Nome Completo",placeholder:"Digite seu nome completo...",inputType:"text",accessibilityLabel:"Digite o seu nome completo..."}),e.jsx(V,{name:"email",onChange:()=>{},isEditing:!1,value:Q.email,label:"Email Institucional",placeholder:"Digite seu nome email...",inputType:"text",accessibilityLabel:"Digite o seu email..."}),e.jsx(V,{name:"password",onChange:(I,B)=>y(B),isEditing:x,value:x?f:"valordeteste",label:x?"Senha Atual":"Senha",placeholder:"Digite sua senha atual...",inputType:"password",accessibilityLabel:"Digite a sua senha..."}),x&&e.jsx(V,{name:"passwordConfirm",onChange:(I,B)=>L(B),label:"Nova Senha",placeholder:"Digite a sua nova senha...",inputType:"password",accessibilityLabel:"Digite a sua nova senha..."}),Ee(),!C&&!x&&e.jsxs("div",{className:"sub-buttons-container",children:[e.jsx(j,{variant:"subtle",size:"md",onPress:()=>l(!0),children:"Sair"}),e.jsx(j,{variant:"subtle",size:"md",onPress:()=>m(!0),children:"Deletar a Conta"})]})]}),e.jsx(ye,{isOpen:u,handleAccept:()=>{l(!1),c(),t("/")},handleClose:()=>l(!1),question:"Tem certeza que deseja sair da aplicação?",acceptText:"Sair do UnBFórum"}),e.jsx(ye,{isOpen:h,handleAccept:()=>{D()},handleClose:()=>m(!1),question:"Tem certeza que deseja deletar sua conta?",acceptText:"Deletar",isLoadingAcceptButton:U,loadingAcceptButtonText:"Deletando..."})]})}const Uo=p.main`
  width: 70%;
  display: flex;
  flex-direction: column;
  align-items: flex-start;
  justify-content: space-between;
  gap: 1rem;
  margin-top: 1rem;

  @media (max-width: 768px) {
    width: 90%;
    flex-direction: column;
    align-items: center;
    justify-content: center;
    margin-top: 0.5rem;
  }
`,Po=p.section`
  width: 100%;
  display: flex;
  flex-direction: column;
  align-items: flex-start;
  gap: 0.5rem;
  /* margin-top: 0rem; */

  @media (max-width: 768px) {
    width: 90%;
    flex-direction: column;
    /* align-items: flex-end;
    justify-content: flex-end; */
    /* margin-top: 1rem; */
  }
`,qo=p.div`
  width: 80%;
  display: flex;
  flex-direction: column;
  align-items: flex-start;
  justify-content: flex-start;

  gap: 1rem;
  margin-left: 10%;

  @media (max-width: 768px) {
    width: 100%;
    margin-left: 5%;
    gap: 0.5rem;
  }
`,Mo=p.div`
  width: 100%;
  display: flex;
  flex-direction: row;
  align-items: flex-start;
  justify-content: flex-start;
`,Fo=p.div`
  width: 100%;
  display: flex;
  flex-direction: column;
  /* gap: 0.3rem; */

  & > div,
  textarea {
    height: ${({isComment:t})=>t?"5rem":"13rem"};
  }

  & > textarea {
    font-size: 0.9rem;
    line-height: 1rem;
    color: ${i.colors.black};
  }

  @media (max-width: 768px) {
    & > div,
    textarea {
      height: ${({isComment:t})=>t?"5rem":"7rem"};
    }
  }
`,Bo=p.label`
  display: flex;
  flex-direction: row;
  gap: 0.3rem;
  font-size: ${({fontSize:t})=>t};
  line-height: 1.5rem;
  color: ${i.colors.text[500]};

  #required {
    color: ${i.colors.danger[600]};
  }

  @media (max-width: 768px) {
    font-size: ${({fontSize:t})=>Number(t.split("rem")[0])-.2+"rem"};
  }
`;function Ae({label:t="",inputType:o="text",placeholder:s,accessibilityLabel:r,size:n="md",fontSize:c="0.9rem",isRequired:d=!0,isEditing:u=!0,name:l,isComment:h=!1,value:m=void 0,onChange:C,...g}){return e.jsxs(Fo,{isComment:h,children:[t&&e.jsxs(Bo,{fontSize:c,children:[t,d&&e.jsx("p",{id:"required",children:"*"})]}),e.jsx(Ke,{type:o,variant:u?"outline":"filled",size:n,placeholder:s,accessibilityLabel:r,editable:u,onChangeText:x=>C(l,x),autoCompleteType:null,value:m,totalLines:5,maxLength:700,...g})]})}const Vo=t=>{let o=!0;const s=[];return(!t||t==="")&&(o=!1,s.push("Comentário")),{isValid:o,fieldErrors:s}},Ro=({topicId:t,onCommentCreated:o})=>{const{name:s}=J(),r=Z(),n=H("(max-width: 768px)"),[c,d]=a.useState(""),[u,l]=a.useState(!1);a.useEffect(()=>console.log({comment:c}),[c]);const h=(C,g)=>{d(g)},m=C=>{C.preventDefault();const{isValid:g}=Vo(c);if(!g){r.show({placement:"top-right",render:()=>e.jsx(v,{id:"create-comment-error",title:"Campos Inválidos",description:"O comentário não pode ser feito sem conteúdo!",status:""})});return}l(!0),Rt(c.trim(),t).then(x=>{d(""),o()}).catch(x=>{r.show({placement:"top-right",render:()=>e.jsx(v,{id:"create-topic-error",title:"Opa",description:"Erro ao criar comentário",status:""})})}).finally(()=>{l(!1)})};return e.jsxs(qo,{children:[e.jsxs(Mo,{children:[e.jsx(ae,{bg:"lime.600",marginRight:"0.8rem",size:n?"xs":"sm",children:ze(s)}),e.jsx(Ae,{name:"content",onChange:h,fontSize:"1.2rem",size:"lg",value:c,isComment:!0,inputType:"text",accessibilityLabel:"Comentário",label:"",placeholder:"Digite o comentário aqui..."})]}),e.jsx(j,{alignSelf:"flex-end",size:n?"sm":"md",rightIcon:e.jsx(Ye,{size:20,color:i.colors.white}),onPress:C=>m(C),isLoading:u,isLoadingText:"Comentando...",children:"Comentar"}),e.jsx(ne,{})]})};function Zo(){const{id:t}=We(),{token:o}=J(),[s,r]=a.useState(null),[n,c]=a.useState(!1),[d,u]=a.useState(!1);a.useEffect(()=>window.scrollTo(0,0),[]),a.useEffect(()=>{t&&(c(!0),It(Number(t)).then(h=>{r(h.data)}).finally(()=>c(!1)))},[t]);const l=a.useCallback(()=>{s&&(u(!0),Zt(s.id).then(h=>{r({...s,comments:h.data})}).finally(()=>{u(!1)}))},[s]);return e.jsx(Uo,{children:s?e.jsxs(e.Fragment,{children:[e.jsx(ee,{id:Number(s.id),author:s.author.name,title:s.title,categories:s.categories,content:s.content,rating:s.rating,files:s.files,currentRating:s.current_user_rating,isSave:s.current_user_has_saved,isInsideTopic:!0}),e.jsxs(Po,{children:[o&&e.jsx(Ro,{topicId:s.id,onCommentCreated:l}),d?e.jsx(_,{accessibilityLabel:"Carregando os comentários do tópico..."}):e.jsx(e.Fragment,{children:s.comments.map(h=>e.jsx(ee,{id:h.id,isComment:!0,topicId:s.id,author:h.author.name,rating:h.rating,currentRating:h.current_user_rating,content:h.content},h.id))})]})]}):e.jsx(_,{accessibilityLabel:"Carregando dados do tópico..."})})}const Xo=p.section`
  position: relative;
  width: 80%;
  height: calc(100vh - 6rem);
  display: flex;
  align-items: center;
  justify-content: center;

  .logo-categories-container {
    width: 30%;

    svg {
      width: 17rem;
      height: 19rem;
    }
  }

  .categories-list {
    width: 60%;
    height: 100%;
    display: flex;
    flex-direction: column;
    justify-content: center;
    align-items: center;
    gap: 1rem;

    .categories-page-title {
      font-weight: bold;
      font-size: 2rem;
    }
  }

  #create-category {
    width: 100%;
    display: flex;
    flex-direction: row;
    align-items: center;
    justify-content: space-between;

    & > div[role='button'] {
      width: 30%;
    }
  }

  @media (max-width: 930px) {
    .logo-categories-container {
      svg {
        width: 15rem;
        height: 13rem;
      }
    }

    #create-category {
      flex-direction: column;
      gap: 0.7rem;

      & > div[role='button'] {
        width: 100%;
      }
    }
  }

  @media (max-width: 950px) {
    .logo-categories-container {
      display: none;
    }

    .categories-list {
      width: 100%;

      .categories-page-title {
        font-weight: bold;
        font-size: 1.5rem;
      }
    }
  }
`,Ho=p.div`
  width: 100%;
  max-height: 70vh;
  display: grid;
  grid-template-columns: repeat(3, 1fr);
  grid-template-rows: auto;
  align-items: center;
  justify-items: center;
  align-content: space-between;
  gap: 1rem;

  overflow-y: auto;
  padding: 0 0 1rem 0;

  #category-card {
    height: 10rem;
    position: relative;
    width: 100%;
    display: flex;
    flex-direction: column;
    align-items: flex-start;
    justify-content: center;

    /* grid-column: 1 / span 2;
    grid-row: 1 / 3; */

    padding: 1rem;
    background-color: ${i.colors.muted[100]};
    border: 1px solid ${i.colors.black+"70"};
    border-radius: 8px;
    box-shadow: 0 0 5px ${i.colors.black+"50"};

    #category-name {
      margin: 1.3rem 0 0 0;
      font-size: 1rem;
      font-weight: bold;
      margin-bottom: 0.5rem;
      color: ${i.colors.primary[700]};
    }
  }

  @media (max-width: 768px) {
    grid-template-columns: repeat(1, 1fr);

    #category-card {
      height: 8rem;
    }
  }
`,Oo=p.span`
  width: 50px;
  height: 20px;
  border-radius: 4px;
  /* Use a placeholder color property; you may replace it with your logic */
  background-color: ${t=>t.colorIndicator}; /* Example color */
  /* You can adjust the size and other properties as needed */
`,Jo=p.div`
  position: absolute;
  top: 0;
  right: 0;
  display: flex;
  flex-direction: row;
`,Go=p.button`
  display: flex;
  align-items: center;
  justify-content: center;

  padding: 0.5rem;
  border-radius: 0 0 0 0;
  transition: 0.3s all ease;

  background-color: ${i.colors.purple[600]};

  &:hover {
    filter: brightness(70%);
  }

  @media (max-width: 768px) {
    padding: 0.5rem;
  }
`,Ko=p.button`
  display: flex;
  align-items: center;
  justify-content: center;

  padding: 0.5rem;
  border-radius: 0 8px 0 0;
  transition: 0.3s all ease;

  background-color: ${i.colors.danger[600]};

  &:hover {
    filter: brightness(70%);
  }

  @media (max-width: 768px) {
    padding: 0.5rem;
  }
`,Yo=p.div`
  position: fixed;
  top: 0;
  left: 0;
  width: 100%;
  height: 100%;
  background-color: ${i.colors.black+"75"};
  display: flex;
  justify-content: center;
  align-items: center;
  opacity: ${({isOpen:t})=>t?"1":"0"};
  visibility: ${({isOpen:t})=>t?"visible":"hidden"};
  transition:
    opacity 0.3s ease,
    visibility 0s linear ${({isOpen:t})=>t?"0s":"0.3s"};
`,Wo=p.section`
  position: relative;
  background-color: white;
  width: 60%;
  max-width: 880px;
  /* height: 80vh; */
  display: flex;
  flex-direction: column;
  align-items: flex-start;
  justify-content: flex-start;
  gap: 1rem;
  overflow: auto;

  border-radius: 8px;
  padding: 1.7rem 3rem;
  box-shadow: 0 0 10px ${i.colors.black+"50"};
  opacity: ${({isOpen:t})=>t?"1":"0"};

  & > h1 {
    font-size: 2rem;
    font-weight: 700;
    line-height: 1.5;
    color: ${i.colors.text[700]};
  }

  @media (max-width: 768px) {
    width: 90%;
    /* height: 90vh; */
    max-width: none;

    padding: 0.5rem 1rem;

    & > h1 {
      font-size: 1.5rem;
      font-weight: 700;
      line-height: 1.5;
    }
  }
`,$o=p.div`
  width: 100%;
  display: flex;
  flex-direction: row;
  align-items: center;
  justify-content: flex-end;
`;p.div`
  width: 100%;
`;const _o=p.button`
  position: absolute;
  top: 0;
  right: 0;

  display: flex;
  align-items: center;
  justify-content: center;

  padding: 0.8rem;
  border-radius: 0 8px 0 0;
  transition: 0.3s all ease;

  background-color: ${i.colors.danger[600]};

  &:hover {
    filter: brightness(70%);
  }

  @media (max-width: 768px) {
    padding: 0.5rem;
  }
`,es=p.div`
  width: 4rem;
  height: 2rem;
  border-radius: 5px;
  box-shadow: 0 0 5px rgba(0, 0, 0, 0.3);

  background-color: ${({color:t})=>t};
`,ts=t=>{let o=!0;const s=[],{name:r,color:n}=t;return(!r||r==="")&&(o=!1,s.push("Nome da Categoria")),(!n||n==="")&&(o=!1,s.push("Cor da Categoria")),{isValid:o,fieldErrors:s}};function os({isModalOpen:t,setIsModalOpen:o,title:s=null,selectedCategory:r=null,isEditing:n=!1}){const c=Z(),d=H("(max-width: 768px)"),[u,l]=a.useState(!1),[h,m]=a.useState(r||{id:0,color:"#10B981",name:""}),[C,g]=a.useState(!1);a.useEffect(()=>console.log(h),[h]),a.useEffect(()=>{l(t),m(r||{id:0,color:"#10B981",name:""})},[t,r]);const x=a.useCallback((S,k)=>{m({...h,[S]:k})},[h]),w=S=>{S.preventDefault();const{isValid:k,fieldErrors:A}=ts(h);k||c.show({placement:"top-right",render:()=>e.jsx(v,{id:"create-topic-error",title:"Campos Inválidos",description:`Os sequintes campos estão incorretos: ${A.reduce((z,E,b)=>b===0?`'${E}'`:b===A.length-1?`${z} e '${E}'`:`${z}, '${E}'`,"")}`,status:""})}),g(!0),(n?bt:Ct)(h).then(z=>{c.show({placement:"top-right",render:()=>e.jsx(v,{id:"create-category-success",title:"Categoria Criada com Sucesso",description:"Agora todos podem utilizar a nova categoria!",status:"success"})}),Q(),m({id:0,color:"#10B981",name:""})}).catch(z=>{c.show({placement:"top-right",render:()=>{let E="";return typeof z.response.data.detail=="object"?E=z.response.data.detail[0].msg.split(", ")[1]:E=z.response.data.detail,e.jsx(v,{id:"create-topic-error",title:"Campos Inválidos",description:`Erro: ${E}`,status:""})}})}).finally(()=>{g(!1)})},Q=()=>{l(!1),setTimeout(()=>{o(!1)},300)};return e.jsx(e.Fragment,{children:u&&e.jsx(Yo,{isOpen:u,onClick:Q,children:e.jsxs(Wo,{isOpen:u,onClick:S=>S.stopPropagation(),children:[e.jsx(_o,{onClick:Q,children:e.jsx(re,{size:"2rem",color:i.colors.primary[50]})}),e.jsx("h1",{children:s||"Nova categoria"}),e.jsx(V,{name:"name",onChange:x,fontSize:"1.2rem",value:h.name||void 0,size:"lg",inputType:"text",accessibilityLabel:"Nome da Categoria",label:"Nome da Categoria",placeholder:"Digite o nome da categoria..."}),e.jsx(V,{name:"color",onChange:x,fontSize:"1.2rem",value:h.color,size:"lg",inputType:"text",accessibilityLabel:"Cor da Categoria (com #)",label:"Cor da Categoria (com #)",placeholder:"Escolha a cor da categoria #000000..."}),e.jsx(es,{color:h.color}),e.jsx($o,{children:e.jsx(j,{id:"create-topic-button",onPress:S=>w(S),bgColor:i.colors.success[600],size:d?"xs":"md",borderRadius:"4px",_text:{fontSize:"1rem",fontWeight:"700"},isLoadingText:"Criando...",isLoading:C,rightIcon:e.jsx(De,{}),children:"Criar"})})]})})})}const ve="Nova Categoria";function ss(){const t=Z(),{checkScopePermissions:o}=J(),s=H("(max-width: 768px)"),[r,n]=a.useState([]),[c,d]=a.useState(null),[u,l]=a.useState(!1),[h,m]=a.useState(ve),[C,g]=a.useState(!1);a.useEffect(()=>{u||x().then()},[u]);const x=async()=>{g(!0),de().then(k=>{n(k.data)}).finally(()=>{g(!1)})},w=a.useCallback(k=>{l(k),d(null),m(ve)},[]),Q=(k,A)=>{k.preventDefault(),g(!0),wt(A).then(U=>{t.show({placement:"top-right",render:()=>e.jsx(v,{id:"delete-category-success",title:"Categoria Apagada com Sucesso",description:"",status:"success"})}),x().then()}).catch(U=>{t.show({placement:"top-right",render:()=>{let z="";return typeof U.response.data.detail=="object"?z=U.response.data.detail[0].msg.split(", ")[1]:z=U.response.data.detail,e.jsx(v,{id:"create-category-error",title:"Erro ao apagar categoria",description:`Erro: ${z}`,status:""})}})}).finally(()=>{g(!1)})},S=(k,A)=>{k.preventDefault(),d(A),m("Editar Categoria"),l(!0)};return e.jsxs(e.Fragment,{children:[e.jsxs(Xo,{children:[e.jsx("div",{className:"logo-categories-container",children:e.jsx(Y,{})}),e.jsxs("section",{className:"categories-list",children:[e.jsxs("div",{id:"create-category",children:[e.jsx("h1",{className:"categories-page-title",children:"Lista de Categorias"}),(o(se)||o(oe))&&e.jsx(j,{size:s?"sm":"lg",rightIcon:e.jsx(le,{}),onPress:()=>w(!0),children:"Criar categoria"})]}),e.jsx(Ho,{children:C?e.jsx(_,{accessibilityLabel:"Carregando os tópicos..."}):e.jsx(e.Fragment,{children:r.map(k=>e.jsxs("div",{id:"category-card",children:[(o(se)||o(oe))&&e.jsxs(Jo,{children:[e.jsx(Go,{onClick:A=>S(A,k),children:e.jsx($e,{color:i.colors.primary[50]})}),e.jsx(Ko,{onClick:A=>Q(A,k.id),children:e.jsx(Se,{color:i.colors.primary[50]})})]}),e.jsx("p",{id:"category-name",children:k.name}),e.jsx(Oo,{colorIndicator:k.color})]},k.id))})})]})]}),(o(se)||o(oe))&&e.jsx(os,{isModalOpen:u,setIsModalOpen:w,selectedCategory:c,isEditing:c!==null,title:h})]})}const is=p.div`
  /* position: fixed;
  top: 0;
  left: 0; */
  width: 100%;
  /* height: 100%; */
  /* background-color: ${i.colors.black+"75"}; fundo escurecido */
  display: flex;
  justify-content: center;
  align-items: center;
  opacity: ${({isOpen:t})=>t?"1":"0"};
  visibility: ${({isOpen:t})=>t?"visible":"hidden"};
  transition:
    opacity 0.3s ease,
    visibility 0s linear ${({isOpen:t})=>t?"0s":"0.3s"};
`,ns=p.section`
  position: relative;
  /* background-color: white; */
  width: 60%;
  max-width: 880px;
  height: 80vh;
  display: flex;
  flex-direction: column;
  align-items: flex-start;
  justify-content: flex-start;
  gap: 1rem;
  /* overflow: auto; */

  border-radius: 8px;
  padding: 1.7rem 3rem;
  /* box-shadow: 0 0 10px ${i.colors.black+"50"}; */
  opacity: ${({isOpen:t})=>t?"1":"0"};
  /* transform: translateY(-50px);
  transition:
    opacity 0.3s ease,
    transform 0.3s ease;
  */
  & > h1 {
    font-size: 2rem;
    font-weight: 700;
    line-height: 1.5;
    color: ${i.colors.text[700]};
  }

  @media (max-width: 768px) {
    width: 90%;
    height: 90vh;
    max-width: none;

    padding: 0.5rem 1rem;

    & > h1 {
      font-size: 1.5rem;
      font-weight: 700;
      line-height: 1.5;
    }
  }
`,rs=p.div`
  width: 100%;
  display: flex;
  flex-direction: row;
  align-items: center;
  justify-content: space-between;
`,as=p.div`
  width: 100%;
`,ls=p.button`
  position: absolute;
  top: 0;
  right: 0;

  display: flex;
  align-items: center;
  justify-content: center;

  padding: 0.8rem;
  border-radius: 0 8px 0 0;
  transition: 0.3s all ease;

  /* background-color: ${i.colors.danger[600]}; */

  &:hover {
    filter: brightness(70%);
  }

  @media (max-width: 768px) {
    padding: 0.5rem;
  }
`,cs=p.div`
  position: relative;
  width: 100%; /* largura desejada */
  display: flex;
  flex-direction: column;
  align-items: flex-start;
  justify-content: flex-start;
  gap: 1rem;

  /* Estilo personalizado para o botão de upload */
  .custom-file-upload {
  }

  /* Estilo para quando o mouse está sobre o botão */
  .custom-file-upload:hover {
    background-color: #e0e0e0;
  }
`,ds=p.label`
  font-size: ${({fontSize:t})=>t};
  line-height: 1.5rem;
  color: ${i.colors.text[500]};

  @media (max-width: 768px) {
    font-size: ${({fontSize:t})=>Number(t.split("rem")[0])-.2+"rem"};
  }
`,us=p.div`
  width: 100%;
  height: 12rem;
  display: flex;
  flex-direction: row;
  align-items: center;
  justify-content: flex-start;
  gap: 1.5rem;
  overflow-x: auto;

  /* background-color: blue; */

  & > p {
    width: 10rem;
    height: 100%;
    display: flex;
    gap: 0.5rem;
    flex-direction: column;
    align-items: center;
    justify-content: center;

    padding: 0.8rem 1rem;

    text-align: center;
    font-weight: 400;
    line-height: 1.5;

    background-color: ${i.colors.tertiary[100]};
  }

  #file {
    position: relative;

    font-size: 0.8rem;
    color: ${i.colors.text[500]};

    overflow: hidden;
    word-wrap: break-word;
  }

  #delete-file-button {
    position: absolute;
    top: 0;
    right: 0;

    padding: 0.3rem;
    background-color: ${i.colors.danger[600]};

    transition: 0.3s all ease;
  }

  #delete-file-button:hover {
    filter: brightness(70%);
  }

  #no-files {
    width: 100%;
    align-items: flex-start;

    font-size: 0.9rem;
    color: ${i.colors.text[500]};
    background-color: transparent;

    padding: 0;
  }

  @media (max-width: 768px) {
    height: 10rem;
    gap: 0.5rem;

    & > p {
      width: 8rem;
      gap: 0.5rem;

      padding: 0.4rem 0.5rem;

      line-height: 1.4;

      background-color: ${i.colors.tertiary[100]};
    }

    #file {
      font-size: 0.7rem;
    }

    #delete-file-button {
      padding: 0.1rem;
    }

    #no-files {
      font-size: 0.7rem;
    }
  }
`,ps=async t=>{const o=new FormData;return o.append("files",t),console.log(o),await N.post("files/upload/",o,{headers:{"Content-Type":t.type}})};function hs({handleAddFile:t,handleDeleteFile:o,files:s,fileInputRef:r,filesUploadLimit:n=2,setIsAddFileLoading:c}){const d=Z(),u=async m=>{const C=m.target.files&&m.target.files[0];C&&s.length+1<=n?(console.log("tentando subir arquivo1"),c(!0),console.log("tentando subir arquivo2"),ps(C).then(g=>{console.log(g.data[0]),t(g.data[0])}).catch(g=>{d.show({placement:"top",render:()=>e.jsx(v,{id:"files-upload-error",status:"error",title:"Opa!",description:"Erro ao fazer upload do arquivo."})})}).finally(()=>{c(!1)})):d.show({placement:"top",render:()=>e.jsx(v,{id:"files-error",status:"error",title:"Opa!",description:"Número de arquivos atingido ou arquivo é inválido."})})},l=()=>{const m=[];return s.forEach((C,g)=>{m.push(e.jsxs("p",{id:"file",children:[e.jsx("button",{id:"delete-file-button",onClick:()=>o(C.name),children:e.jsx(Se,{size:"1.7rem",color:i.colors.primary[50]})}),e.jsx(_e,{size:"38",color:i.colors.gray[500]}),C.name]},g))}),m},h=()=>e.jsx("p",{id:"no-files",children:"Sem arquivos adicionados"});return e.jsx(e.Fragment,{children:e.jsxs(cs,{children:[e.jsx(ds,{fontSize:"1.2rem",children:`Arquivos (${s.length}/${n} arquivos adicionados)`}),e.jsx("input",{ref:r,id:"file-upload",type:"file",style:{display:"none"},onChange:u}),e.jsx(us,{children:s.length>0?l():h()})]})})}const ms=t=>{let o=!0;const s=[],{title:r,content:n}=t;return(!r||r==="")&&(o=!1,s.push("Título")),(!n||n==="")&&(o=!1,s.push("Conteúdo")),{isValid:o,fieldErrors:s}},gs=t=>{const{title:o,content:s,files:r,categories:n}=t;return{title:o,content:s,categories:n,files:r.map(c=>c.id)}};function xs({isModalOpen:t,setIsModalOpen:o}){const s=Z(),r=H("(max-width: 768px)"),n=a.useRef(null),[c,d]=a.useState(!1),[u,l]=a.useState({title:"",content:"",files:[],categories:[]}),[h,m]=a.useState(!1),[C,g]=a.useState(!1),[x,w]=a.useState([]),Q={control:f=>({...f,backgroundColor:i.colors.white}),option:(f,{data:y,isMulti:T,isDisabled:L,isFocused:F,isSelected:D})=>{const M=K(y.color);return{...f,backgroundColor:L?void 0:D?T?y.color:M.alpha(.2).css():F?M.alpha(.1).css():void 0,color:L?"#ccc":D&&T?K.contrast(M,"white")>2?"white":"black":y.color,cursor:L?"not-allowed":"default",":active":{...f[":active"],backgroundColor:L?void 0:D?y.color:M.alpha(.3).css()}}},multiValue:(f,{data:y})=>{const T=K(y.color);return{...f,backgroundColor:T.alpha(.1).css()}},multiValueLabel:(f,{data:y})=>({...f,color:y.color}),multiValueRemove:(f,{data:y})=>({...f,color:y.color,":hover":{backgroundColor:y.color,color:"white"}}),input:f=>({...f,"input:focus":{boxShadow:"none"}})};a.useEffect(()=>console.log(u),[u]),a.useEffect(()=>{de().then(f=>{w(f.data)})},[]),a.useEffect(()=>{d(t),l({title:"",content:"",files:[],categories:[]})},[t]);const S=a.useCallback((f,y)=>{Array.isArray(y)?l({...u,[f]:y.map(T=>Number(T.value))}):l({...u,[f]:y})},[u]),k=a.useCallback(f=>{g(f)},[]),A=f=>{f.preventDefault();const{isValid:y,fieldErrors:T}=ms(u);if(!y){s.show({placement:"top-right",render:()=>e.jsx(v,{id:"create-topic-error",title:"Campos Inválidos",description:`Os sequintes campos estão incorretos: ${T.reduce((L,F,D)=>D===0?`'${F}'`:D===T.length-1?`${L} e '${F}'`:`${L}, '${F}'`,"")}`,status:""})});return}m(!0),Ut(gs(u)).then(L=>{s.show({placement:"top-right",render:()=>e.jsx(v,{id:"create-topic-success",title:"Tópico Criado com Sucesso",description:"Agora todos podem ver seu novo tópico!",status:"success"})}),U(),l({title:"",content:"",files:[],categories:[]})}).catch(L=>{s.show({placement:"top-right",render:()=>{let F="";return typeof L.response.data.detail=="object"?F=L.response.data.detail[0].msg.split(", ")[1]:F=L.response.data.detail,e.jsx(v,{id:"create-topic-error",title:"Campos Inválidos",description:`Erro: ${F}`,status:""})}})}).finally(()=>{m(!1)})},U=()=>{d(!1),setTimeout(()=>{o(!1)},300)},z=a.useCallback(f=>{l({...u,files:[...u.files,f]})},[u]),E=a.useCallback(f=>{const y=u.files.filter(T=>T.name!==f);l({...u,files:y})},[u]),b=()=>{n&&n.current&&n.current.click()};return e.jsx(e.Fragment,{children:e.jsx(is,{isOpen:c,children:e.jsxs(ns,{isOpen:c,onClick:f=>f.stopPropagation(),children:[e.jsx(ls,{onClick:U,children:e.jsx(et,{size:"2rem",color:i.colors.tertiary[400]})}),e.jsx("h1",{children:"Novo Tópico"}),e.jsx(V,{name:"title",onChange:S,fontSize:"1.2rem",size:"lg",inputType:"text",accessibilityLabel:"Título",label:"Título",placeholder:"Digite o título do tópico..."}),e.jsx(Ae,{name:"content",onChange:S,fontSize:"1.2rem",size:"lg",inputType:"text",accessibilityLabel:"Conteúdo",label:"Conteúdo",placeholder:"Digite o conteúdo do tópico aqui..."}),e.jsx(as,{children:e.jsx(ie,{isMulti:!0,closeMenuOnSelect:!1,options:x.map(f=>({value:f.id.toString(),label:f.name,color:f.color})),styles:Q,onChange:f=>S("categories",f),placeholder:"Adicionar categorias"})}),e.jsx(hs,{filesUploadLimit:2,fileInputRef:n,handleAddFile:z,handleDeleteFile:E,files:u.files,setIsAddFileLoading:k}),e.jsxs(rs,{children:[e.jsx(j,{onPress:()=>b(),size:r?"xs":"md",isLoading:C,isLoadingText:"Adicionando arquivo...",borderRadius:"4px",_text:{fontSize:"1rem",fontWeight:"400"},rightIcon:e.jsx(le,{}),children:"Adicionar arquivos"}),e.jsx(j,{id:"create-topic-button",onPress:f=>A(f),bgColor:i.colors.success[600],size:r?"xs":"md",isDisabled:C,borderRadius:"4px",_text:{fontSize:"1rem",fontWeight:"700"},isLoadingText:"Criando...",isLoading:h,rightIcon:e.jsx(De,{}),children:"Criar"})]})]})})})}function fs(){const t=O();return e.jsx(e.Fragment,{children:e.jsx(xs,{isModalOpen:!0,setIsModalOpen:()=>t("/")})})}const Cs=p.section`
  position: relative;
  width: 80%;
  height: calc(100vh - 6rem);
  display: flex;
  flex-direction: row;
  align-items: center;
  /* justify-content: center; */

  .logo-saved-topics-container {
    width: 30%;

    svg {
      width: 17rem;
      height: 19rem;
    }
  }

  .saved-topics-list {
    width: 60%;
    height: 100%;
    display: flex;
    flex-direction: column;
    justify-content: center;
    align-items: center;
    gap: 1rem;
  }

  .saved-topics-title {
    font-weight: bold;
    font-size: 2rem;
  }

  & > div[role='button'] {
    width: 30%;
  }

  .reload-saved-topics {
    width: 100%;
    display: flex;
    flex-direction: row;
    align-items: center;
    justify-content: space-between;
  }

  @media (max-width: 930px) {
    .logo-saved-topics-container {
      svg {
        width: 15rem;
        height: 13rem;
      }
    }

    .reload-saved-topics {
      flex-direction: column;
      gap: 0.7rem;

      & > div[role='button'] {
        width: 100%;
      }
    }
  }

  @media (max-width: 950px) {
    .logo-saved-topics-container {
      display: none;
    }

    .saved-topics-list {
      width: 100%;
    }

    .saved-topics-title {
      font-weight: bold;
      font-size: 1.5rem;
    }
  }
`,ws=p.div`
  width: 100%;
  max-height: 70vh;
  display: flex;
  flex-direction: column;
  grid-template-rows: auto;
  align-items: center;
  justify-items: center;
  align-content: space-between;
  gap: 1rem;

  overflow-y: auto;
  padding: 0 0 1rem 0;

  @media (max-width: 768px) {
    grid-template-columns: repeat(1, 1fr);
  }
`;function bs(){const t=H("(max-width: 768px)"),[o,s]=a.useState(!1),[r,n]=a.useState([]);a.useEffect(()=>{c().then()},[]);const c=async()=>{s(!0),Bt().then(d=>{n(d.data)}).finally(()=>{s(!1)})};return e.jsx(e.Fragment,{children:e.jsxs(Cs,{children:[e.jsx("div",{className:"logo-saved-topics-container",children:e.jsx(Y,{})}),e.jsxs("section",{className:"saved-topics-list",children:[e.jsxs("div",{className:"reload-saved-topics",children:[e.jsx("h1",{className:"saved-topics-title",children:"Meus Tópicos Salvos"}),e.jsx(j,{size:t?"sm":"lg",rightIcon:e.jsx(tt,{size:22,color:i.colors.white}),onPress:c,children:"Recarregar"})]}),e.jsx(ws,{children:o?e.jsx(_,{accessibilityLabel:"Carregando os tópicos salvos..."}):e.jsx(e.Fragment,{children:r.map(d=>e.jsx(ee,{id:d.id,title:d.title,rating:d.rating,files:d.files,currentRating:d.current_user_rating,content:d.content,author:d.author.name,commentsCount:d.comments_count,categories:d.categories,isSave:!0},d.id))})})]})]})})}function js(){return e.jsxs(ot,{children:[e.jsxs(q,{path:"/",element:e.jsx($,{}),children:[e.jsx(q,{path:"/",element:e.jsx(_t,{})}),e.jsx(q,{path:"/topic/:id",element:e.jsx(Zo,{})})]}),e.jsx(q,{path:"/profile",element:e.jsx($,{}),children:e.jsx(q,{path:"/profile",element:e.jsx(Io,{})})}),e.jsx(q,{path:"/categories",element:e.jsx($,{}),children:e.jsx(q,{path:"/categories",element:e.jsx(ss,{})})}),e.jsx(q,{path:"/create-topic",element:e.jsx($,{}),children:e.jsx(q,{path:"/create-topic",element:e.jsx(fs,{})})}),e.jsx(q,{path:"/saved-topics",element:e.jsx($,{}),children:e.jsx(q,{path:"/saved-topics",element:e.jsx(bs,{})})}),e.jsxs(q,{path:"/login",element:e.jsx(mo,{}),children:[e.jsx(q,{path:"/login",element:e.jsx(uo,{})}),e.jsx(q,{path:"/login/logon",element:e.jsx(no,{})}),e.jsx(q,{path:"/login/create-account",element:e.jsx(So,{})})]})]})}const ys={white:"#FFF","gray-100":"#E1E1E6","gray-300":"#C4C4CC","gray-400":"#8D8D99","gray-500":"#7C7C8A","gray-600":"#323238","gray-700":"#29292E","gray-800":"#202024","gray-900":"#121214","green-300":"#00B37E","green-500":"#00875F","green-700":"#015F43","red-500":"#AB222E","red-700":"#7A1921","yellow-500":"#FBA94C"},vs=st`
  * {
    margin: 0;
    padding: 0;
    outline: 0;
    box-sizing: border-box;
  }

  :focus {
    outline: 0;
    /* box-shadow: 0 0  10px ${t=>t.theme["green-500"]}; */
  }

  body {
    background: ${i.colors.dark[900]};
    color: ${i.colors.black};
    overflow-y: auto;
  }

  body, input, textarea, button, div[role='button'] {
    font-family: 'Poppins', 'Montserrat', sans-serif;
    font-weight: 400;
    font-size: 1rem;
    transition: background-color .3s;
  }

  button {
    cursor: pointer;
    /* font-size: 1.6rem; */
    background: none;
    border: 0;
  }

  h1, h2, h3, h4, h5 {
    line-height: 1.5;
    font-weight: 600;
    /* color: #000; */
  }

  #reactionButton {
    box-shadow: none;

    &:hover {
      & > svg {
        color: ${i.colors.tertiary[500]}
      }
    }
  }

  /* Chrome, Safari, Edge, Opera */
  input::-webkit-outer-spin-button,
  input::-webkit-inner-spin-button {
    -webkit-appearance: none;
    margin: 0;
  }

  /* Firefox */
  input[type=number] {
    -moz-appearance: textfield;
  }
`;function Ns(){const t={get:async()=>{try{return localStorage.getItem("@UnBForum:theme")==="dark"?"dark":"light"}catch(o){return console.log(o),"light"}},set:async o=>{try{localStorage.setItem("@UnBForum:theme",o)}catch(s){console.log(s)}}};return e.jsx(it,{colorModeManager:t,children:e.jsx(nt,{theme:ys,children:e.jsxs(Vt,{children:[e.jsx(rt,{children:e.jsx(js,{})}),e.jsx(vs,{})]})})})}at.createRoot(document.getElementById("root")).render(e.jsx(lt.StrictMode,{children:e.jsx(Ns,{})}));
